...
CTableFrameSink::CTableFrameSink()
{
	...
	CString str;
	str.Format("server: CTableFrameSink 37---%+I64d,%+I64d,%+I64d,%+I64d", m_llMuscle[0],m_llMuscle[1],m_llMuscle[2],m_llMuscle[3]);
...
void __cdecl CTableFrameSink::RepositTableFrameSink()
{
	...
	CString str;
	str.Format("server: RepositTableFrameSink 140---%+I64d,%+I64d,%+I64d,%+I64d", m_llMuscle[0],m_llMuscle[1],m_llMuscle[2],m_llMuscle[3]);
	OutputDebugString(str);
...
void CTableFrameSink::InitData()
{
	...
	CString str;
	str.Format("server: InitData 216---%+I64d,%+I64d,%+I64d,%+I64d", m_llMuscle[0],m_llMuscle[1],m_llMuscle[2],m_llMuscle[3]);
	OutputDebugString(str);
...
bool __cdecl CTableFrameSink::OnEventGameStart()
{
	...
	CString str;
	str.Format("server: OnEventGameStart() 538-- m_wCurrentUser %d", m_wCurrentUser);
	OutputDebugString(str);
...
{
	CString str;
	str.Format("server: OnEventGameEnd 637---cbReason %d---%+I64d,%+I64d,%+I64d,%+I64d", cbReason, m_llMuscle[0],m_llMuscle[1],m_llMuscle[2],m_llMuscle[3]);
	OutputDebugString(str);
	...
	case GER_NORMAL:		//常规结束
		{
			...
			CString str2;
			str2.Format("server: OnEventGameEnd 678--m_byGameEndType %d,(%d,%d,%d,%d)", m_byGameEndType,m_ChiHuResult[0].wChiHuKind,m_ChiHuResult[1].wChiHuKind,
			...
			{
				OutputDebugString("server: OnEventGameEnd : 有人胡牌");
				//有人胡牌
				//自摸类型
				if ((1==m_byGameEndType || 3==m_byGameEndType)   &&   (m_ChiHuResult[m_wProvideUser].wChiHuKind!=CHK_NULL))
				{
					//自摸,除海底自摸
					OutputDebugString("server: OnEventGameEnd : 有人胡牌 自摸");
					////翻数计算
				...
				{
					OutputDebugString("server: OnEventGameEnd 763: 有人胡牌 放炮");
			...
			{
				OutputDebugString("server: OnEventGameEnd : 荒庄处理");
				//荒庄处理
			...
			CString str1;
			str1.Format("server: OnEventGameEnd 865--(%I64d,%I64d,%I64d,%I64d)", GameEnd.lGameScore[0],GameEnd.lGameScore[1],
			...
			for(int i=0;i<GAME_PLAYER;i++)
			{
				if(lUserScore[i]>0)
				{
					lRevenue=lUserScore[i]*(m_pGameServiceOption->wRevenue/100.0f);
				...
				CString str;
				str.Format("server: OnEventGameEnd 899---%d,(%I64d,%I64d),%d", i,GameEnd.lGameScore[i],lRevenue,ScoreKind);
				OutputDebugString(str);
			...
			CString str;
			str.Format("server: OnEventGameEnd 909---%d,%d", m_pGameServiceOption->wRevenue,m_wProvideUser);
			OutputDebugString(str);
			str.Format("server: OnEventGameEnd 889---lGameScore 游戏积分,%+I64d,%+I64d,%+I64d,%+I64d", GameEnd.lGameScore[0],GameEnd.lGameScore[1],GameEnd.lGameScore[2],GameEnd.lGameScore[3]);
			OutputDebugString(str);
			str.Format("server: OnEventGameEnd 890---iCellScore 服务费,%+I64d,%+I64d,%+I64d,%+I64d", GameEnd.iCellScore[0],GameEnd.iCellScore[1],GameEnd.iCellScore[2],GameEnd.iCellScore[3]);
			OutputDebugString(str);
			str.Format("server: OnEventGameEnd 891---iChiHuKind 胡牌类型,%d,%d,%d,%d", GameEnd.iChiHuKind[0],GameEnd.iChiHuKind[1],GameEnd.iChiHuKind[2],GameEnd.iChiHuKind[3]);
			OutputDebugString(str);
			str.Format("server: OnEventGameEnd 892---cbTimes 番数,%d,%d,%d,%d", GameEnd.cbTimes[0],GameEnd.cbTimes[1],GameEnd.cbTimes[2],GameEnd.cbTimes[3]);
			OutputDebugString(str);
			str.Format("server: OnEventGameEnd 893---iMuscle 筋,%d,%d,%d,%d", GameEnd.iMuscle[0],GameEnd.iMuscle[1],GameEnd.iMuscle[2],GameEnd.iMuscle[3]);
			OutputDebugString(str);
	...
	case GER_USER_LEFT:		//用户强退
		{
			...
			CString str;
			str.Format("server: OnEventGameEnd 949---%+I64d,%+I64d,%+I64d,%+I64d", GameEnd.lGameScore[0],GameEnd.lGameScore[1],GameEnd.lGameScore[2],GameEnd.lGameScore[3]);
			OutputDebugString(str);
			...
			for (WORD i=0;i<m_wPlayerCount;i++)
			{
				...
				{
					GameEnd.iCellScore[i] = GameEnd.lGameScore[i]*(0.01*m_pGameServiceOption->wRevenue);
				}
			}

			CString str1;
			str1.Format("server: OnEventGameEnd 989-- 发送消息 SUB_S_GAME_END 强退");
			OutputDebugString(str1);
...
bool __cdecl CTableFrameSink::OnGameMessage(WORD wSubCmdID, const void * pDataBuffer, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	strBuffer.Format("server: CTableFrameSink::OnGameMessage() wSubCmdID=%d, wDataSize=%d, wChairID=%d", wSubCmdID, wDataSize, pUserData->wChairID); 
...
bool CTableFrameSink::OnWriteMuscleScore(WORD wWinChairID, WORD wLostChairID, int iMuscle, LONGLONG llResultGold[])
{
	...
	{
		CString strBuffer;
		strBuffer.Format("Error: CTableFrameSink::OnWriteMuscleScore(WORD,WORD,int) wWinChairID=%d, wLostChairID=%d, iMuscle=%d", 
	...
	llResultGold[wWinChairID] = iMuscle*m_pGameServiceOption->lCellScore*(1-0.01*m_pGameServiceOption->wRevenue);//扣税
...
bool CTableFrameSink::OnWriteMuscleScore(WORD wWinChairID, int iMuscle, LONGLONG llResultGold[])
{
	...
	{
		CString strBuffer;
		strBuffer.Format("Error: CTableFrameSink::OnWriteMuscleScore(WORD,int) wWinChairID=%d, iMuscle=%d", wWinChairID, iMuscle);
		OutputDebugString(strBuffer);
		return false;
	}
	...
	//case WIK_TING:			//听牌操作
		{
			...
			if (m_cbCardIndex[wChairID][cbCardIndex]==1)
			{
				...
				if ( !bBrightMuscleCanHu )
				{
					...
					CString str;
					str.Format("server: OnUserOperateCard 1641-%d-----%+I64d,%+I64d,%+I64d,%+I64d", wChairID, m_llMuscle[0],m_llMuscle[1],m_llMuscle[2],m_llMuscle[3]);
					OutputDebugString(str);
				}
			}
			...
			{//杠的情况
				...
				CString str;
				str.Format("server: OnUserOperateCard 1670-%d-----%+I64d,%+I64d,%+I64d,%+I64d", wChairID, m_llMuscle[0],m_llMuscle[1],m_llMuscle[2],m_llMuscle[3]);
				OutputDebugString(str);
			}
...
bool CTableFrameSink::OnOtherUserOperateCard(WORD wChairID, BYTE cbOperateCode, BYTE cbOperateCard)
{
	...
	else if (WIK_NULL == cbTargetAction) //虽然玩家有操作，但是都是放弃
	{
		...
		if (m_OperateResult.bBringMuscleCanHu)
		{
			...
			CString str;
			str.Format("server: OnUserOperateCard 1641-明筋能胡-终于扣金币了-%d-----%+I64d,%+I64d,%+I64d,%+I64d", m_OperateResult.wOperateUser, m_llMuscle[0],m_llMuscle[1],m_llMuscle[2],m_llMuscle[3]);
			OutputDebugString(str);
	...
	case WIK_GANG:		//杠牌操作
		//case WIK_TING:		//听牌操作
		{
			...
			CString str;
			str.Format("server: OnUserOperateCard 1512-%d,%d-----%+I64d,%+I64d,%+I64d,%+I64d", wTargetUser, m_WeaveItemArray[wTargetUser][wIndex].wProvideUser,
				m_llMuscle[0],m_llMuscle[1],m_llMuscle[2],m_llMuscle[3]);
			OutputDebugString(str);

			str.Format("server: OnUserOperateCard 1749------%+I64d,%+I64d,%+I64d,%+I64d", llReturnMuscleGold[0],llReturnMuscleGold[1],llReturnMuscleGold[2],llReturnMuscleGold[3]);
			OutputDebugString(str);
			break;
		}
	}
...
bool CTableFrameSink::DispatchCardData(WORD wCurrentUser, bool bSupplyCard)
{
	...
	CString strBuffer;
	strBuffer.Format("Server: CTableFrameSink::DispatchCardData 剩余牌数目:%d",m_cbLeftCardCount);
	OutputDebugString(strBuffer);
