...
void CHeapCard::DrawCardControl(CDC * pDC)
{
	...
	//static unsigned __int64 lltemp=0;
	//lltemp++;
	//unsigned __int64 llret=lltemp%200;

	switch (m_CardDirection)
	{
	case Direction_East:	//东向（视图ID=1，右）
		{
			//绘画扑克
			if ((m_wFullCount-m_wMinusHeadCount-m_wMinusLastCount)>0)
			{
				...
				static POINT PointOff[14]={{-4,2},{1,12},{6,22},{11,32},{17,42},{23,54},{28,66},{35,79},{43,93},{52,108},{59,125},{70,143},{80,163},{92,184}};

				//头部扑克
				if (m_wMinusHeadCount%2!=0)
				{
					ASSERT(wHeapIndex>=0 && wHeapIndex<14);
					nXPos=m_ControlPoint.x+PointOff[wHeapIndex].x;
					nYPos=m_ControlPoint.y+PointOff[wHeapIndex].y;
				...
				for (int i=wDoubleHeap;i<wFinallyIndex;i++)
				{
					...
					ASSERT(i>=0 && i<14);
					nXPos=m_ControlPoint.x+PointOff[i].x;
					nYPos=m_ControlPoint.y+PointOff[i].y;
				...
				if (m_wMinusLastCount%2!=0)
				{
					...
					ASSERT(wFinallyIndex>=0 && wFinallyIndex<14);
					nXPos=m_ControlPoint.x+PointOff[wFinallyIndex].x;
					nYPos=m_ControlPoint.y+PointOff[wFinallyIndex].y;
	...
	case Direction_South:	//南向（视图ID=2，底）
		{
			//绘画扑克
			if ((m_wFullCount-m_wMinusHeadCount-m_wMinusLastCount)>0)
			{
				...
				if (m_wMinusLastCount%2!=0)
				{
					...
					{
						//nXPos=m_ControlPoint.x+35*(wFinallyIndex+5)+4;
                        nXPos=m_ControlPoint.x+35*(13-wFinallyIndex)+4; 
						nYPos=m_ControlPoint.y;
						m_ImageHeapSingleHB.DrawImage(pDC,nXPos,nYPos,nWidth,nHeight,nWidth*(13-wFinallyIndex),0,nWidth,nHeight);
					}
	...
	case Direction_West:	//西向（视图ID=3，左）
		{
			//绘画扑克
			if ((m_wFullCount-m_wMinusHeadCount-m_wMinusLastCount)>0)
			{
                ...
                static POINT PointOffWest[14]={{2,-11},{-3,-1},{-8,9},{-13,19},{-17,29},{-23,41},{-30,54},{-37,67},{-45,81},{-52,96},{-61,113},{-70,131},{-80,151},{-92,172}};

				//尾部扑克
				if (m_wMinusLastCount%2!=0)
				{
					ASSERT(wFinallyIndex>=0 && wFinallyIndex<14);
					nXPos=m_ControlPoint.x+PointOffWest[13-wFinallyIndex].x;
					nYPos=m_ControlPoint.y+PointOffWest[13-wFinallyIndex].y;
					m_ImageHeapSingleVL.DrawImage(pDC,nXPos,nYPos,nWidth,nHeight,nWidth*wFinallyIndex,0,nWidth,nHeight);
				}

				//中间扑克
				//for (int i=wDoubleHeap;i<wFinallyIndex;i++)//不可用这句
				for (int i=wFinallyIndex-1;i>=wDoubleHeap;i--)
				{
					...
					ASSERT(i>=0 && i<14);
					nXPos=m_ControlPoint.x+PointOffWest[13-i].x;
					nYPos=m_ControlPoint.y+PointOffWest[13-i].y;
				...
				if (m_wMinusHeadCount%2!=0)
				{
					int i=14-wDoubleHeap;
					ASSERT(i>=0 && i<14);
					nXPos=m_ControlPoint.x+PointOffWest[i].x;
					nYPos=m_ControlPoint.y+PointOffWest[i].y;
	...
	case Direction_North:	//北向（视图ID=0，顶）
		{
			//绘画扑克
			if ((m_wFullCount-m_wMinusHeadCount-m_wMinusLastCount)>0)
			{
	...
	case Direction_East:	//东向
		{
			...
			for (WORD i=0;i<=num;i++)
			{
				...
				{//胡牌
					nXPos=m_ControlPoint.x-32-7*(13-i)-5;
					nYPos=m_ControlPoint.y+i*18 - 12;
	...
	case Direction_West:	//西向
		{
			...
			for (WORD i=13-num;i<=13;i++)
			{
				...
				else if (i ==13 && 2 == (m_wCardCount%3))
				{
					nXPos=m_ControlPoint.x+9*(13-i)+15-8;
					nYPos=m_ControlPoint.y+i*21-49-68 + 50-37;
					int nImageXPos=0;
					int nImageYPos=(GetImageIndex(m_cbCardData[m_wCardCount-1])-1)*nItemHeight;
			...
			else  if (1 == count%3)             //没胡13
			{
				if (count > 7) //后七张
				{
					for(int i = count; i>6; --i)
					{
						...
						int nYPos=m_ControlPoint.y;
						int nImageXPos=i*nItemWidth;
						int nImageYPos=(GetImageIndex(m_cbCardData[m_wCardCount-i])-1)*nItemHeight;
						m_ImageUserTableCardHT.DrawImage(pDC,nXPos,nYPos,nWidth,nHeight,nWidth*i,0);
						m_ImageTabCardNumHT.DrawImage(pDC,nXPos,nYPos,nWidth,nHeight,nImageXPos,nImageYPos);
					}
					count -= (count -6);
...
void CTableCard::SetWinCardData(const BYTE byCardData)
{
	// 将胡牌数据在牌数组的位置
	BYTE byIndex=255, bySwap=255;
	for (BYTE i=0; i<m_wCardCount; i++)
	{
		if (byCardData == m_cbCardData[i])
		{
			byIndex = i;
		}
	}
	...
	if (255 != byIndex)
	{
		bySwap = m_cbCardData[byIndex];
		//// 不同方位，位置不同，所以需要区分
		//if (Direction_North == m_CardDirection)
		//{
		//	for (BYTE i=byIndex; i>0; i--)
		//	{
		//		if (i-1 <= 0)
		//			break;

		//		m_cbCardData[i-1] = m_cbCardData[i];
		//	}

		//	m_cbCardData[0] = bySwap;
		//}
		//else //除了北方位，其他三个方位绘制时取数组值的顺序都是一样的。
		{
			for (BYTE i=byIndex; i<m_wCardCount; i++)
			{
				if (i+1 >= m_wCardCount)
					break;

				m_cbCardData[i] = m_cbCardData[i+1];
			}

			m_cbCardData[m_wCardCount-1] = bySwap;
		}
	}
	return;
}
...
void CCardControl::SetSelectStatus(bool bSelectStatus, BYTE byCardDataArray[]/*=NULL*/, BYTE byCardCount/*=0*/)
{
	m_bSelectStatus = bSelectStatus;

	for (WORD i=0; i<m_wCardCount; i++)
	{
		m_CardItemArray[i].bSelectFlag = FALSE;
		if ( bSelectStatus )
		{
			for (WORD j=0; j<byCardCount; j++)
			{
				if (byCardDataArray[j] == m_CardItemArray[i].cbCardData)
				{
					m_CardItemArray[i].bSelectFlag = TRUE;
					break;
				}
			}
		}
...
bool CCardControl::GetHoverCardItem(tagCardItem &CardItem)
{
	...
	if (m_wHoverItem!=INVALID_ITEM)
	{
		if (m_wHoverItem==CountArray(m_CardItemArray))
		{
			...
			CardItem.bSelectFlag = m_CurrentCard.bSelectFlag;
			return true;
		}
		else
		{
			...
			CardItem.bSelectFlag = m_CardItemArray[m_wHoverItem].bSelectFlag;
			return true;
		}
	}
...
bool CCardControl::SetCurrentCard(tagCardItem CardItem)
{
	...
	m_CurrentCard.bSelectFlag = CardItem.bSelectFlag;
	return true;
}
...
bool CCardControl::SetCardData(const BYTE cbCardData[], WORD wCardCount, BYTE cbCurrentCard/*,BYTE cbKingCard*/)
{
	...
	//	for(WORD i=0;i<m_wCardCount;i++)
	//	{
	//		if(CardItemArray[i].bKingFlag==true)//王牌靠前
	//		{
...
bool CCardControl::SetCardItem(const tagCardItem CardItemArray[], WORD wCardCount)
{
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		m_CardItemArray[i].bSelectFlag = CardItemArray[i].bSelectFlag;
	}
...
void CCardControl::DrawCardControl(CDC * pDC)
{
	...
	for (WORD i=0;i<m_wCardCount;i++)
	{
		...
		if ( !m_CardItemArray[i].bShoot   &&   i!=m_wHoverItem )
		{
			nYScreenPos += POS_SHOOT;
		}

		if (m_bSelectStatus)
		{
			//选择杠，碰状态时，如果是相同的牌，一起弹起
			if (INVALID_ITEM!=m_wHoverItem && m_CardItemArray[i].cbCardData==m_CardItemArray[m_wHoverItem].cbCardData)
			{
				nYScreenPos = m_ControlPoint.y;
			}
		}
		...
		if(m_bSelectStatus && !m_CardItemArray[i].bSelectFlag)
		{
...
bool CCardControl::OnEventSetCursor(CPoint Point, bool & bRePaint)
{
	...
	if ((wHoverItem!=m_wHoverItem)&&(m_bPositively==true))
	{
		if (m_bSelectStatus)
		{
			if (!m_CardItemArray[wHoverItem].bSelectFlag)
				return false;
