...
void CGameClient::InitSome(DWORD dwWndID)
{
	...
	if(S_OK==hResult && m_pRoomLog)
	{
		...
		strTmp.Format(".\\ZZMJAndroidLog\\%d\\",dwWndID);
		strPath = strPath.Left(strPath.Find("AndroidLoad.exe")) + strTmp;//ZZMJAndroid.dll

		m_pRoomLog->Init();
		m_pRoomLog->Start();
		m_pRoomLog->Register(strPath, 50000);
		m_pRoomLog->AddMsg(0, 0, TRUE,"***** 转转麻将机器人日志开始服务");
	}
}
...
void CGameClient::UninitSome()
{
	...
	{
		if (m_pRoomLog)
			m_pRoomLog->AddMsg(0, 0, TRUE,"***** 转转麻将机器人日志停止服务");
...
void CGameClient::OnUserOperateCard()
{
	...
	{//没有听牌
		...
		//{//开始听牌
		//	if (m_byGangCardNums>0 && 0!=m_byBuCard[0])
		//	{
		//		OnCardOperate(WIK_TING, m_byBuCard[0]);
		//		return;
		//	}
		//}
		else if (0<(WIK_GANG&m_cbActionMask)  ||  0<(WIK_FILL&m_cbActionMask)) //有杠必杠，因为能赢钱
		{
			if (0 < m_byBuCardNums)
			{
				OnCardOperate(WIK_GANG, m_byBuCard[0]);
				return;
			}
		}
		else if (0 < (WIK_PENG&m_cbActionMask))//有碰必碰，因为杠能赢钱
		{
			OnCardOperate(WIK_PENG, m_byBuCard[0]);
			return;
		}
