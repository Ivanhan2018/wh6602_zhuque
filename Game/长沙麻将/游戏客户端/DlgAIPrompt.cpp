...
void CDlgAIPrompt::OnPaint()
{
	...
	if (NULL != m_memdc.m_hDC)
	{
		...
		if (!m_ImageCenter.IsNull())
		{
			...
			{
				m_ImageCenter.DrawImage(&m_memdc, iCenterX, ptStart.y);
			}
			int iLessW = (ptStart.x+iWidth-m_iAbs_Right)-iCenterX;
			if (iLessW > 0)
			{
				m_ImageCenter.DrawImage(&m_memdc, iCenterX, ptStart.y, iLessW, m_iAbs_Center_Y, 0, 0, iLessW, m_iAbs_Center_Y);
			}
		}
		//左
		if (!m_ImageLeft.IsNull())
		{
			m_ImageLeft.DrawImage(&m_memdc, ptStart.x, ptStart.y);
		}
		//右
		if (!m_ImageRight.IsNull())
		{
			m_ImageRight.DrawImage(&m_memdc, ptStart.x+iWidth-m_iAbs_Right, ptStart.y);
		}
		...
		if (!m_ImageCard.IsNull())
		{
			...
			if (iCardNums > 0)
			{
				for (int i=0; i<iCardNums; ++i)
				{
					...
					m_ImageCardBack.DrawImage(&m_memdc, ptStart.x+i*(IMJWIDTH+2)+12, ptStart.y+8, IMJWIDTH, IMJHEIGHT, 0, 0, IMJWIDTH, IMJHEIGHT, m_fCardAlpha);
					m_ImageCard.DrawImage(&m_memdc, ptStart.x+i*(IMJWIDTH+2)+10, ptStart.y+12, 32, 35, cbCardIndex*32, 0, 32, 35, m_fCardAlpha);
				}
			}
		}
		UpdateRgn(&m_memdc);
		dc.BitBlt(0, 0, m_iCurWidth, m_iCurHeight, &m_memdc, 0, 0, SRCCOPY);
	}
}
