...
bool __cdecl CAndroidUserItemSink::OnEventTimer(UINT nTimerID)
{
	...
	case IDI_OPERATE_HAI_DI://海底操作
		{
			m_pIAndroidUserItem->KillGameTimer(IDI_OPERATE_HAI_DI);

			//构造消息
			CMD_C_OperateHaiDi OperateHaiDi;
			OperateHaiDi.cbTakeHaiDi=0;
			m_pIAndroidUserItem->SendSocketData(SUB_C_OPERATE_HAI_DI,&OperateHaiDi,sizeof(OperateHaiDi));
...
bool __cdecl CAndroidUserItemSink::OnEventGameMessage(WORD wSubCmdID, void * pData, WORD wDataSize)
{
	...
	case SUB_S_OPERATE_HAI_DI:	//海底操作				
		{
			return OnSubOperateHaiDi(pData,wDataSize);
		}
	}
...
//海底操作
bool CAndroidUserItemSink::OnSubOperateHaiDi(const void * pData, WORD wDataSize)
{
	//效验数据
	...
	//消息处理
	CMD_S_OperateHaiDi * pOperateHaiDi=(CMD_S_OperateHaiDi *)pData;

	//设置状态
	m_pIAndroidUserItem->SetGameStatus(GS_MJ_HAI_DI);
	...
	//时间设置
	WORD wMeChairId = m_pIAndroidUserItem->GetChairID();
	if (wMeChairId==pOperateHaiDi->wCurrentUser)
	{
		UINT nElapse = rand()%TIME_OPERATE_HAI_DI+TIME_LESS+5;
		m_pIAndroidUserItem->SetGameTimer(IDI_OPERATE_HAI_DI,nElapse);
	}
