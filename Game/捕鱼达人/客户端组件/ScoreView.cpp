...
void CFishScoreView::OnPaint() 
{
	...
	if (true==m_bShowNumber)
	{
		...
		for (enBulletCountKind BulletCountKind=enBulletCountKind_100; BulletCountKind<enBulletCountKind_End; BulletCountKind=enBulletCountKind(BulletCountKind+1))
			lLeftCoin+=((m_lBulletCharge[BulletCountKind]/lBulletCount[BulletCountKind])*m_lBulletCount[BulletCountKind]);
		DrawNumber(&dcMemory,90,302,lLeftCoin*m_lCellScore,false);
		DrawNumber(&dcMemory,230,302,m_lCoinCount*m_lCellScore,false);
		DrawNumber(&dcMemory,372,302,0L,false);
		DrawNumber(&dcMemory,134,328,(m_lCoinCount+lLeftCoin)*m_lCellScore);
		DrawNumber(&dcMemory,320,328,0L);
	}
...
BOOL CFishScoreView::OnInitDialog()
{
	...
	m_btCalcScore.Create(NULL,WS_CHILD|WS_VISIBLE,CRect(10,10,20,30),this,IDC_SWITCH_CHAIR);
	m_btContinueGame.Create(NULL,WS_CHILD|WS_VISIBLE,CRect(10,10,20,30),this,IDC_CONTINUE_GAME);

	//按钮图片
	HINSTANCE hInstance=AfxGetInstanceHandle();
	m_btQuitGame.SetButtonImage(IDB_BT_QUIT_GAME_EX,hInstance,4,false);
	m_btCalcScore.SetButtonImage(IDB_BT_CALC_SCORE,hInstance,4,false);
	m_btContinueGame.SetButtonImage(IDB_BT_CONTINUE_GAME,hInstance,4,false);
	...
	{ 
		...
		if(fun) fun(this->GetSafeHwnd(),RGB(0,23,35),240,2);
		FreeLibrary(hInst); 
	} 
	...
	//设置定时
	SetTimer(IDI_ALPHA_DEPTH,150,NULL);
...
void CFishScoreView::OnSize(UINT nType, int cx, int cy)
{
	...
	DeferWindowPos(hDWP,m_btQuitGame,NULL,35,cy-34,0,0,uFlags);
	DeferWindowPos(hDWP,m_btCalcScore,NULL,35+1*(rcButton.Width()+nSpace),cy-34,0,0,uFlags);
	DeferWindowPos(hDWP,m_btContinueGame,NULL,35+2*(rcButton.Width()+nSpace),cy-34,0,0,uFlags);
...
//设置信息
void CFishScoreView::SetScoreInfo(LONG lBulletCount[enBulletCountKind_End],WORD wCptrFishCount[enFishKind_No],LONGLONG lCoinCount, LONG lBulletCharge[enBulletCountKind_End])
{
	//设置信息
	CopyMemory(m_lBulletCount,lBulletCount,sizeof(m_lBulletCount));
