	...
	case IDI_OUT_CARD:		//用户出牌
		{
			...
			if (OutCardResult.cbCardCount>0)
			{
				...
				m_pIAndroidUserItem->SendSocketData(SUB_C_OUT_CART,&OutCard,sizeof(CMD_C_OutCard));
...
bool CAndroidUserItemSink::OnSubCallScore(void * pData, WORD wDataSize)
{
	...
	if (pCallScore->bPreCallScorePhase==CSD_BRIGHTCARD && pCallScore->bCallScorePhase==CSD_BRIGHTCARD) bUserBrightCard=true;
