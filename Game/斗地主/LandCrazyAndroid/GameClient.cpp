...
//抢地主			
#define				IDI_GT_GAME_USER_SNATCH					3011
	...
	case IDI_GT_GAME_USER_JIAOFEN:
		{//叫分定时器
			if (0==nElapse|| nElapse>100)
			{
				...
				{
					LandScore.bLandScore = 1;	//直接叫3分			Update:<cxf>; Description:<叫地主:1;不叫:0> (之前是叫3分);
				}
				else
				{
					LandScore.bLandScore = 0;	//不叫			Update:<cxf>; Description:<叫地主:1;不叫:0> (之前是255分;255代表不叫);
				}
				SendData(SUB_C_LAND_SCORE,&LandScore,sizeof(LandScore));
			}
			break;
		}
	case IDI_GT_GAME_USER_SNATCH:
		{//抢地主
			KillGameTimer(IDI_GT_GAME_USER_SNATCH);
			CMD_C_SnatchLand SnatchLand;
			if (m_GameLogic.GetLandTag())
			{
				SnatchLand.bSnatchLand = 1;	//抢地主			Update:<cxf>; Description:<抢地主:1;不抢:0> 
			}
			else
			{
				SnatchLand.bSnatchLand = 0;	//不抢				Update:<cxf>; Description:<抢地主:1;不抢:0>
			}
...
bool CGameClient::OnGameMessage(WORD wSubCmdID, const void * pBuffer, WORD wDataSize)
{
	...
	case SUB_S_USER_SNATCH_SCORE:	//抢地主
		{
			bRet = OnSubSnatchLand(pBuffer,wDataSize);
			break;
		}
	...
	if (NULL != pLandScore)
	{
		...
		if (pLandScore->wCurrentUser != INVALID_CHAIR)
		{
			if (pLandScore->wCurrentUser==wMeChairID)
			{
				...
				if (0 == pLandScore->bLandScore)		//Author:<cxf>;
				{//叫分
					SetGameTimer(IDI_GT_GAME_USER_JIAOFEN, iTimeRand);
				}
				else
				{//抢地主
					SetGameTimer(IDI_GT_GAME_USER_SNATCH, iTimeRand);
				}
			}
		}
	}
...
//Description:<抢地主>;
bool CGameClient::OnSubSnatchLand(const void *pBuffer,WORD wDataSize)
{
	//效验数据
	ASSERT(wDataSize==sizeof(CMD_S_SnatchLand));
	if (wDataSize!=sizeof(CMD_S_SnatchLand))
	{
		return false;
	}
	...
	if (NULL != pSnatchLand)
	{
		...
		//设置变量
		m_cbCallScorePhase = pSnatchLand->bCallScorePhase;

		WORD wMeChairID = GetMeChairID();
		if (pSnatchLand->wCurrentUser != INVALID_CHAIR)
		{
			if (pSnatchLand->wCurrentUser==wMeChairID)
			{
				int iTimeRand = rand()%4+1;
				SetGameTimer(IDI_GT_GAME_USER_SNATCH, iTimeRand);
			}
...
bool CGameClient::OnSubGameStartOutCard(const void * pBuffer, WORD wDataSize)
{
	...
	if (NULL != pGameStart)
	{
		...
		{//如果我是当前玩家，则出牌。
			...
			//增加三张底牌
			CopyMemory(&m_bHandCardData[17],pGameStart->bBackCardData,sizeof(BYTE)*3);
			m_bHandCardCount+=3;
			m_GameLogic.SetHandCard(m_bHandCardData, m_bHandCardCount, true);

			int iTimeRand = rand()%3 + 2;
...
void CGameClient::GameAutoLastOneCard()
{
	...
	if (m_bHandCardCount>=1 && m_bHandCardCount<=20)
	{
		OutCard.bCardData[0] = m_bHandCardData[m_bHandCardCount-1];//出最后一张牌
	}
	else
	{
		OutCard.bCardData[0] = m_bHandCardData[0];//出第一张牌
	}
	//发送出牌信息
...
break;
			...
			if (m_bJiaBeiTag>=1 && m_bJiaBeiTag<=3)
			{
				...
				DoubleScore.bDoubleType=m_bJiaBeiTag;
				//发送消息
