...
CCardControl::CCardControl()
{
	...
	if (m_bLoad==false)
	{
		...
		m_PngCard.LoadImage(strPath+TEXT("\\LandCrazy\\picture\\CARD.png"));
...
void CCardControl::RectifyControl()
{
	...
	for (DWORD i=0;i<dwCardCount;i++)
	{
		...
		//合并区域
		SignedRegion.CreateRoundRectRgn(nXPos,nYPos,nXPos+m_CardSize.cx+1,nYPos+m_CardSize.cy+1,10,10);
...
void CCardControl::OnPaint()
{
	...
	CMemDC dcMem(&dc, rcClient, false);
	...
	for (DWORD i=0;i<dwCardCount;i++)
	{
		...
		//绘画扑克
		m_PngCard.DrawImage( &dcMem, dwXScreenPos, dwYScreenPos,m_CardSize.cx, m_CardSize.cy, dwXImagePos, dwYImagePos);

		//绘画选择
		if ((m_wStartIndex<=i)&&(i<=m_wEndIndex))
		{
			CSize SizeSelectImage(m_ImageCardSelect.GetWidth(),m_ImageCardSelect.GetHeight());
			m_ImageCardSelect.DrawImage(&dcMem,dwXScreenPos,dwYScreenPos,73,99,0,0);
...
int CCardControl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_ImageCardSelect.LoadImage(TEXT("LandCrazy\\picture\\CARD_SELECT.png"));
	return 0;
}
