#include "StdAfx.h"
#include "Resource.h"
#include "CardControlBack.h"
#include ".\cardcontrolback.h"

//////////////////////////////////////////////////////////////////////////
//静态变量

bool					CCardControlBack::m_bLoad=false;					//加载标志
CSize					CCardControlBack::m_CardSize;						//扑克大小
//CSkinImage				CCardControl::m_ImageCard;						//图片资源
CPngImage				CCardControlBack::m_PngCard;						//图片资源

//////////////////////////////////////////////////////////////////////////

BEGIN_MESSAGE_MAP(CCardControlBack, CWnd)
	ON_WM_PAINT()
	ON_WM_CREATE()
	ON_WM_SETCURSOR()
	ON_WM_RBUTTONUP()
...
//构造函数
CCardControlBack::CCardControlBack()
{
	//配置变量
	m_bDisplay=false;
	m_bHorizontal=true;
	...
	if (m_bLoad==false)
	{
		...
		m_PngCard.LoadImage(hInstance,TEXT("CARD_800"));

		//获取大小
		//CImageHandle CardImageHandle(&m_ImageCard);
		m_CardSize.cx=m_PngCard.GetWidth()/13;
		m_CardSize.cy=m_PngCard.GetHeight()/5;
	}
...
//设置窗口
void CCardControlBack::SetSinkWindow(CWnd * pSinkWindow)
{
	//设置变量
...
//设置方向
void CCardControlBack::SetDirection(bool bHorizontal)
{
	//状态判断
...
//弹起扑克
void CCardControlBack::ShootAllCard(bool bShoot)
{
	//变量定义
	bool bRectify=false;
...
//设置显示
void CCardControlBack::SetDisplayFlag(bool bDisplay)
{
	//状态判断
...
//设置响应
void CCardControlBack::SetPositively(bool bPositively)
{
	//设置变量
...
//设置间距
void CCardControlBack::SetCardSpace(DWORD dwCardHSpace, DWORD dwCardVSpace, DWORD dwShootAltitude)
{
	//设置变量
...
//基准位置
void CCardControlBack::SetBenchmarkPos(int nXPos, int nYPos, enXCollocateMode XCollocateMode, enYCollocateMode YCollocateMode)
{
	//设置变量
...
//基准位置
void CCardControlBack::SetBenchmarkPos(const CPoint & BenchmarkPos, enXCollocateMode XCollocateMode, enYCollocateMode YCollocateMode)
{
	//设置变量
...
//获取数目
DWORD CCardControlBack::GetCardCount()
{
	return (DWORD)m_CardDataItem.GetCount();
}

//获取扑克
DWORD CCardControlBack::GetShootCard(BYTE bCardData[], DWORD dwMaxCount)
{
	//变量定义
...
//设置扑克
DWORD CCardControlBack::SetCardData(const BYTE bCardData[], DWORD dwCardCount)
{
	//设置扑克
...
//获取扑克
DWORD CCardControlBack::GetCardData(tagCardItem CardItemArray[], DWORD dwMaxCount)
{
	//效验参数
...
//设置扑克
DWORD CCardControlBack::SetCardItem(const tagCardItem CardItemArray[], DWORD dwCardCount)
{
	//设置扑克
	m_CardDataItem.SetSize(dwCardCount);
...
//设置弹起扑克
DWORD CCardControlBack::SetShootCard(const BYTE bCardDataIndex[], DWORD dwCardCount)
{
	//变量定义
...
//调整位置
void CCardControlBack::RectifyControl()
{
	//变量定义
	DWORD dwCardCount=(DWORD)m_CardDataItem.GetCount();
...
//索引切换
DWORD CCardControlBack::SwitchCardPoint(const CPoint & MousePoint)
{
	if (m_bHorizontal==true)
	{
		//变量定义
...
void CCardControlBack::OnPaint()
{
	...
	for (DWORD i=0;i<dwCardCount;i++)
	{
		...
		//绘画扑克
		m_PngCard.DrawImage( &BufferDC, dwXScreenPos, dwYScreenPos,m_CardSize.cx, m_CardSize.cy, dwXImagePos, dwYImagePos);
...
//建立消息
int CCardControlBack::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
//鼠标消息
void CCardControlBack::OnRButtonUp(UINT nFlags, CPoint Point)
{
...
//鼠标消息
void CCardControlBack::OnLButtonUp(UINT nFlags, CPoint Point)
{
...
//鼠标消息
void CCardControlBack::OnLButtonDown(UINT nFlags, CPoint Point)
{
...
//光标消息
BOOL CCardControlBack::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT message)
{
	//设置光标
...
void CCardControlBack::OnMouseMove(UINT nFlags, CPoint point)
{
