...
int CSendCard::GetCardData(int iChairID, BYTE* pCard, int iNums)
{
	if(!m_strFilePath.IsEmpty())
	{
		...
		char chCardData[1024]={0};
		CString strKeyValue="";
		strKeyValue.Format("User%d", iChairID);
		...
		if (uiReturn>0)
		{
			...
			{
				int iCountCard = 0;
				CString strCardData="", strCardValue="";
				...
				while(!strCardData.IsEmpty())
				{
					...
					if (!strCardValue.IsEmpty() && iSize>0)
					{
						...
						if (strCardValue.GetLength()>=4)
						{
							...
							if (0!=byCardValue && 255!=byCardValue)
							{
