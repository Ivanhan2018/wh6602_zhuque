...
__ENTER_FUNCTION
	...
	//WeaveItem[1].cbWeaveKind = WIK_GANG;

	//tagChiHuResult result;
	//memset(&result, 0, sizeof(tagChiHuResult));
	//BYTE byKingCard=0x29;
	//BYTE cbReCardIndex[MAX_INDEX]={0};
	//BYTE byCardData[7]={0x12,0x13,0x14,0x08,0x08,0x25,0x25};
	//BYTE m_cbCardIndex[MAX_INDEX]={0};
	//m_GameLogic.SwitchToCardIndex(byCardData,7,m_cbCardIndex);
	//BYTE byHuType = m_GameLogic.AnalyseChiHuLevity(m_cbCardIndex,WeaveItem,2,0x25,0,result,&byKingCard,1,cbReCardIndex, true);
	//BYTE bySaveHuType = byHuType;
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	//设置标题
	//SetWindowText(TEXT("益阳麻将  --  Ver：1.0.5.3"));
	...
	strPicPath.Append(TEXT("\\SparrowYY\\Picture\\PNG\\Cartoon\\"));
	...
	case GS_MJ_HAI_DI:	//海底状态
		{
			...
			if (m_wCurrentUser==byMeChairID && cbGameStation!=GS_MJ_HAI_DI)
			{
				...
				CardItem.bKingFlag = (pStatusPlay->cbCardData[0] == m_cbTempKing[0])?true:false;
			...
			//绘画王牌
			m_GameClientView.m_KingCard.SetCardData(&pStatusPlay->byKindCard,1);
			m_GameClientView.m_KingCard.SetDirection(Direction_South);
			m_GameClientView.m_bKingCard=true;
...
__ENTER_FUNCTION
	...
	//char cbData[128]={0};
	//sprintf(cbData,"上一轮出牌玩家:%d, 数据:%x",m_wOutCardUser, m_cbOutCardData);
	//InsertSystemString(cbData);
...
__ENTER_FUNCTION
	...
	/*char chPrintData[255] = {0};
	sprintf(chPrintData, "Me:%d, 当前玩家:%d, 扑克数据:%x, 当前命令:%d", GetMeChairID(), pSendCard->wCurrentUser, pSendCard->cbCardData, pSendCard->cbActionMask);
	InsertShtickString(chPrintData, RGB(255, 255, 0), true);*/
	...
	if (pSendCard->cbCardData!=0)
	{
		...
		else
		{
			...
			CardItem.bShoot = false;
			CardItem.bKingFlag = (pSendCard->cbCardData == m_cbTempKing[0])?true:false;
			m_GameClientView.m_HandCardControl.SetCurrentCard(CardItem);
...
__ENTER_FUNCTION
	...
	{//被动玩家，吃、碰的情况
		...
		if (GetMeChairID()==wOperateUser)
		{
			...
			CardItem.bShoot = false;
			CardItem.bKingFlag = (cbCardData[cbCardCount-1] == m_cbTempKing[0])?true:false;
			m_GameClientView.m_HandCardControl.SetCurrentCard(CardItem);
...
__ENTER_FUNCTION
	...
	//设置控件
	m_GameClientView.m_bKingCard=false;
	m_bIsMeCanOutCard = false;
	m_GameClientView.SetStatusFlag(false,false);
	m_GameClientView.m_btHaiDi.ShowWindow(SW_HIDE);
	m_GameClientView.m_btNoHaiDi.ShowWindow(SW_HIDE);
	...
	GameScoreInfo.cbKingCard=pGameEnd->bKingCard;
	GameScoreInfo.wBankerUser=m_wBankerUser;
	GameScoreInfo.iCellScore=pGameEnd->iCellScore[MyChairId];
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		GameScoreInfo.cbKingCount[i]=pGameEnd->cbKingCount[i];
	...
	if (!pGameEnd->bIsHuangzhuang)
	{
		...
		//服务费
		_snprintf(szBuffer,CountArray(szBuffer),TEXT("\n服务费:%d"),GameScoreInfo.iCellScore);
		InsertGeneralString(szBuffer,RGB(255,255,0),true);
		...
		if (!bIsLeft)
		{
			...
			else
			{
				if(m_bZM)
				{//自摸
					const tagUserData *pUserData = GetUserData(GameScoreInfo.wProvideUser);//用户信息
					WORD wViewChairID=SwitchViewChairID(GameScoreInfo.wProvideUser);
					m_GameClientView.SetUserAction(wViewChairID,WIK_CHI_HU);
					m_byUserAction[GameScoreInfo.wProvideUser] = WIK_CHI_HU;
				...
				{//放炮
					...
					if(byMeChairID==GameScoreInfo.wProvideUser)
					{//本人放炮（别人胡了）
					...
					else
					{
						...
						{//自己胡牌了
							...
							//放炮方的动画
							m_GameClientView.PlayFangPaoFlash(SwitchViewChairID(GameScoreInfo.wProvideUser));
						...
						{//自己没有胡牌.
							...
							PlayGameSoundLanguage(pOtherUserData->cbGender,TEXT("chihu"),false);
						}
					}
				}
			}
...
__ENTER_FUNCTION
	...
	m_GameClientView.m_UserCard[2].SetCardData(0,false);
	m_GameClientView.m_HandCardControl.SetCardData(NULL,0,0,NULL);
	m_GameClientView.m_KingCard.SetCardData(NULL,0);
...
__ENTER_FUNCTION
	...
	PlayGameSoundLanguage(pUserData->cbGender,TEXT("Banker"),false);
...
__ENTER_FUNCTION
	...
	m_GameClientView.m_KingCard.SetCardData(&m_GameStartData.cbKingCard,1);
	m_GameClientView.m_KingCard.SetDirection(Direction_South);
	m_GameClientView.m_bKingCard=true;
...
__ENTER_FUNCTION
	if (!szSoundRes.IsEmpty())
	{
		...
		strPath.Append("\\SparrowYY\\Sound\\");
