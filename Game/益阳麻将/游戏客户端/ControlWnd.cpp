...
__ENTER_FUNCTION
	...
	//绘画背景
	ULONG ulHeight = 0;
	m_ImageControlTop.DrawImage(&memdc,0,0);
	...
	do 
	{
		...
		if (iTempE-iTempS >= iCenterH)
		{
			...
			m_ImageControlMid.DrawImage(&memdc,0,iTempS);
		}
		else
		{
			iTempHeight = iTempE-iTempS;
			m_ImageControlMid.DrawImage(&memdc,0,iTempS,iCenterW, iTempHeight, 0, 0,iCenterW, iTempHeight);
			break;
		}
		iTempS += iTempHeight;
	} while (iTempS<iTempE);
	m_ImageControlBottom.DrawImage(&memdc, 0, rcClient.Height()-m_ImageControlBottom.GetHeight());
	...
	for (BYTE i=0;i<CountArray(cbItemKind);i++)
	{
		if ((m_cbActionMask&cbItemKind[i])!=0)
		{
			...
			{
				m_ImageActionBlack.DrawImage(&memdc,5,nYPos);
			}
			...
			for (BYTE j=0;j<3;j++)
			{
				...
				m_ImageActionCardBack.DrawImage(&memdc,j*28+12+16,nYPos+2);
				m_ImageActionCardNum.DrawImage(&memdc,j*28+10+16,nYPos+7,nWidth,nHeight,(GetImageIndex(cbCardData)-1)*nWidth,0);
			}
			...
			m_ImageActionExplain.DrawImage(&memdc,136,nYPos+5,nItemWidth,nItemHeight,nXImagePos,0);

			//绘画边框
			if (cbCurrentItem==m_cbCurrentItem)
			{
				memdc.Draw3dRect(3,nYPos-2,172,ITEM_HEIGHT-4,RGB(255,255,0),RGB(255,255,0));
			}
	...
	for (BYTE i=0; i<m_sgldCardNums; i++)
	{
		if (0 != m_sgldCardData[i].byCardData)
		{
			...
			for (BYTE j=0;j<4;j++)
			{
				m_ImageActionCardBack.DrawImage(&memdc,j*28+12,nYPos+2);
				m_ImageActionCardNum.DrawImage(&memdc,j*28+10,nYPos+7,nWidth,nHeight,(GetImageIndex(m_sgldCardData[i].byCardData)-1)*nWidth,0);
			}
			...
			if (0 == m_sgldCardData[i].byTag)
			{
				...
				int nItemWidth=m_ImageActionExplain.GetWidth()/5;
				int nItemHeight=m_ImageActionExplain.GetHeight();
				m_ImageActionExplain.DrawImage(&memdc,136,nYPos+5,nItemWidth,nItemHeight,nItemWidth*3,0);
			}
			else if (1 == m_sgldCardData[i].byTag)
			{
				...
				int nItemWidth=m_ImageActionExplain.GetWidth()/5;
				int nItemHeight=m_ImageActionExplain.GetHeight();
				m_ImageActionExplain.DrawImage(&memdc,132,nYPos+5,nItemWidth,nItemHeight,nItemWidth*2,0);
			}
	...
	if ((m_cbActionMask&WIK_CHI_HU)!=0)
	{
		...
		m_ImageActionCardBack.DrawImage(&memdc,12+48,nYPos+2);
		m_ImageActionCardNum.DrawImage(&memdc,10+48,nYPos+7,nWidth,nHeight,byCardIndex*nWidth,0);
