...
CImageNumsYY::CImageNumsYY(void)
{
	m_lNums			=	0;		//数字
...
//设置数字
void CImageNumsYY::SetNums(long lNums)
{
	m_lNums = lNums;
	lNums = abs(lNums);
	m_vecNumArray.clear();
...
//设置有无符号
void CImageNumsYY::SetSign(bool bIsSign)
{
	m_bIsSign = bIsSign;
}

//设置分隔符号
void CImageNumsYY::SetSeparator(bool bIsSeparator)
{
	m_bIsSeparator = bIsSeparator;
}

//设置对齐试
void CImageNumsYY::SetNumAlign(NumAlign align)
{
	m_enNumAlign = align;
}

//设置数字显示区域
void CImageNumsYY::SetShowRect(CRect rect)
{
	m_rcNumsArea = rect;
}

//设置分隔符的宽与高
void CImageNumsYY::SetSeparatorWH(int iWidth, int iHeight)
{
...
//设置图片
void CImageNumsYY::SetImageResource(char* pResource)
{
__ENTER_FUNCTION
	if (NULL != pResource)
	{
...
//绘制数字
void CImageNumsYY::DrawImageNums(CDC* pDC)
{
__ENTER_FUNCTION
	if (!m_ImageNums.IsNull())
	{
