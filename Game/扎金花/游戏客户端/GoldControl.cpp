...
void CGoldControl::OnPaint()
{
	...
	for (int i=0;i<3;i++)
	{
        ...
        else if(m_lGoldCount[0] == 500)iX=5;
		else if(m_lGoldCount[0] == 1000)iX=6;
		else if(m_lGoldCount[0] == 2000)iX=8;
		else if(m_lGoldCount[0] == 10000)iX=9;
		else if(m_lGoldCount[0] == 100000)iX=12;
		else if(m_lGoldCount[0] == 1000000)iX=15;
		m_ImageMoney.AlphaDrawImage(&BackFaceDC,(i)*(m_ImageMoney.GetWidth()/18+5)+17,m_ImageMoney.GetHeight()/2+17,
	...
	while (lGold>0)
	{
		...
		m_ImageNumber.DrawImage(&BackFaceDC,nXExcursion-m_ImageNumber.GetWidth()/10*(iCount++)-15,5,
			m_ImageNumber.GetWidth()/10,m_ImageNumber.GetHeight(),
			m_ImageNumber.GetWidth()/10*(int)lTemp,0);
		lGold/=10;
	}
