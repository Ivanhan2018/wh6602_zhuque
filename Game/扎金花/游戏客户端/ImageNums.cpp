...
//设置图片
void CImageNums::SetImageResource(char* pResource)
{
	if (NULL != pResource)
	{
		...
		m_ImageNums.LoadImage(hInstance,chResource);//数字图片
		...
		if (lBitNums>0)
		{
			...
			{//绘制数字
				if (m_vecNumArray[i]>=0 && m_vecNumArray[i]<=9)
				{
					...
					{//绘制分隔符号
						iX-=m_lSepatatorW;
						m_ImageNums.DrawImage(pDC, iX, m_rcNumsArea.top, m_lSepatatorW, m_lSepatatorH, 12*m_lNumWidth, 0, m_lSepatatorW, m_lSepatatorH);
						iCount3Point = 0;
						--iCountSeparatorNums;
					}
					iX-=m_lNumWidth;
