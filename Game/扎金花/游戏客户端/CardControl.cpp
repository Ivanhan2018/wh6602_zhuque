...
CCardControl::CCardControl()
{
	...
	if (m_bLoad==false)
	{
		//加载资源
		m_bLoad=true;
		m_ImageCard.LoadImage(AfxGetInstanceHandle(),"CARD");
...
void CCardControl::OnPaint()
{
	...
	//BufferDC.SetBkColor(RGB(255, 0, 255));
...
void CCardControl::RectifyControl()    
{
	...
	SetWindowPos(NULL/*&wndBottom*/,nXPos,nYPos,ControlSize.cx,ControlSize.cy,SWP_NOZORDER);
	...
	{
		SignedRegion.CreateRoundRectRgn(i*m_dwCardHSpace,0,i*m_dwCardHSpace+m_CardSize.cx+1,m_CardSize.cy+1,10,10);
...
void CCardControl::OnLButtonUp(UINT nFlags, CPoint point)
{
	if(m_wControlID!=INVALID_CHAIR && m_bCompareCard)
	{
		//用户ID
		//TCHAR obj[32]=TEXT("");
		//_snprintf(obj,sizeof(obj),TEXT("%d:%d"),m_wMessageID,m_wControlID);
		//MessageBox(obj);
