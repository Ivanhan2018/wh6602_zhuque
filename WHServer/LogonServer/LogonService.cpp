...
LRESULT CLogonService::OnSoapMessage(WPARAM wParam, LPARAM lParam)
{
	int wLen;
	switch (wParam)
	{
	case SUB_GP_USER_MALL_BUY_REQ:
		{
			wLen = sizeof(CMD_GP_UserMallBuyReq);
		}
		break;
	case SUB_GP_USER_MALL_UPDATE_REQ:
		{
			wLen = sizeof(CMD_GP_UserMallUpdateReq);
		}
		break;
	default:
		break;
	...
	m_AttemperEngineSink.OnSoapRequest(wParam, (void*)lParam, wLen);	
