...
//	tagGameType * pGameType=NULL;
	...
	{
		tagGameType * pGameType=m_ServerListInfoActive.m_GameTypePtrArrayActive[i];
		tagGameType gameType;
		gameType.wSortID = pGameType->wSortID;
		gameType.wTypeID = pGameType->wTypeID;
		memcpy(gameType.szTypeName, pGameType->szTypeName, TYPE_LEN);

		memset(m_cbSendBuffer, 0, sizeof(m_cbSendBuffer));
		bostream bos;
		bos.attach(m_cbSendBuffer, sizeof(m_cbSendBuffer));
		bos << gameType;

		m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GP_SERVER_LIST,SUB_GP_LIST_TYPE,m_cbSendBuffer,bos.length());
	}
/*	for (DWORD i=0;i<dwTypeCount;i++)
	{
...
//	tagGameKind * pGameKind=NULL;
	...
	for (DWORD i=0;i<dwKindCount;i++)
	{
		...
		gameKind.dwMaxVersion = pGameKind->dwMaxVersion;
		gameKind.dwOnLineCount = pGameKind->dwOnLineCount;
		gameKind.wKindID = pGameKind->wKindID;
		gameKind.wSortID = pGameKind->wSortID;
		gameKind.wTypeID = pGameKind->wTypeID;
		memcpy(gameKind.szKindName, pGameKind->szKindName, KIND_LEN);
		memcpy(gameKind.szProcessName, pGameKind->szProcessName, MODULE_LEN);
		...
		m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GP_SERVER_LIST,SUB_GP_LIST_KIND,m_cbSendBuffer,bos.length());
	}
/*	for (DWORD i=0;i<dwKindCount;i++)
	{
...
//	tagGameStation * pGameStation=NULL;
	...
	{
		tagGameStation * pGameStation=m_ServerListInfoActive.m_GameStationPtrArrayActive[i];
		tagGameStation gameStation;
		gameStation.wStationID = pGameStation->wStationID;
		gameStation.wKindID = pGameStation->wKindID;
		gameStation.wSortID = pGameStation->wSortID;
		gameStation.wJoinID = pGameStation->wJoinID;
		memcpy(gameStation.szStationName, pGameStation->szStationName, STATION_LEN);
		...
		m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GP_SERVER_LIST,SUB_GP_LIST_STATION,m_cbSendBuffer,bos.length());
	}
/*
	for (DWORD i=0;i<dwStationCount;i++)
	{
...
//	tagGameServer * pGameServer=NULL;
	...
	{
		tagGameServer * pGameServer=m_ServerListInfoActive.m_GameServerPtrArrayActive[i];
		tagGameServer gameServer;
		gameServer.dwOnLineCount = pGameServer->dwOnLineCount;
		gameServer.dwServerAddr = pGameServer->dwServerAddr;
		gameServer.wServerID = pGameServer->wServerID;
		gameServer.wServerPort = pGameServer->wServerPort;
		gameServer.wKindID = pGameServer->wKindID;
		gameServer.wSortID = pGameServer->wSortID;
		gameServer.wStationID = pGameServer->wStationID;
		memcpy(gameServer.szServerName, pGameServer->szServerName, SERVER_LEN);
		memcpy(gameServer.szRule, pGameServer->szRule, sizeof(pGameServer->szRule));
//		in_addr serverIP;
//		serverIP.S_un.S_addr = gameServer.dwServerAddr;
//		char * IP = inet_ntoa(serverIP);		
//		memcpy(gameServer.szServerAddr, IP, lstrlen(IP));
		memcpy(gameServer.szServerAddr, pGameServer->szServerAddr, sizeof(pGameServer->szServerAddr));
		...
		m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GP_SERVER_LIST,SUB_GP_LIST_SERVER,m_cbSendBuffer,bos.length());
	}
/*	for (DWORD i=0;i<dwServerCount;i++)
	{
...
//	tagGameServer * pGameServer=NULL;
	...
	for (DWORD i=0;i<dwServerCount;i++)
	{
		...
		if (pGameServer->wKindID==wKindID)
		{
			...
			gameServer.wStationID = pGameServer->wStationID;
			memcpy(gameServer.szServerName, pGameServer->szServerName, SERVER_LEN);
			memcpy(gameServer.szServerAddr, pGameServer->szServerAddr, SERVER_LEN);
