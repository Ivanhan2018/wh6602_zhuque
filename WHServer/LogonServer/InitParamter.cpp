...
void CInitParamter::LoadInitParamter()
{
	...
	//监听端口
	//根据不同的主站来读取不同端口
	memset(szRegName, 0, MAX_PATH);
	_snprintf(szRegName,sizeof(szRegName),TEXT("%s_Port"),m_szMainPage);
	//lErrorCode=RegParamter.QueryValue(TEXT("ListenPort"),&dwDataType,&dwReadData,&dwDataSize);
	dwDataSize=sizeof(dwReadData);
	lErrorCode=RegParamter.QueryValue(szRegName,&dwDataType,&dwReadData,&dwDataSize);
	if (lErrorCode==ERROR_SUCCESS) m_wListenPort=(WORD)dwReadData;
	else m_wListenPort=0;

	//数据库端口
	memset(szRegName, 0, MAX_PATH);
	_snprintf(szRegName,sizeof(szRegName),TEXT("%s_UserDataBasePort"),m_szMainPage);
	dwDataSize=sizeof(dwReadData);
	lErrorCode=RegParamter.QueryValue(szRegName,&dwDataType,&dwReadData,&dwDataSize);
...
void CInitParamter::SaveInitParamter(bool bSaveAll)
{
	...
	//	RegParamter.SetDWORDValue(TEXT("ListenPort"),m_wListenPort);
		...
		memset(szRegName, 0, MAX_PATH);
		_snprintf(szRegName,sizeof(szRegName),TEXT("%s_Port"),m_szMainPage);
		RegParamter.SetDWORDValue(szRegName,m_wListenPort);

		
		//根据不同的主站来读取不同端口
		memset(szRegName, 0, MAX_PATH);
		_snprintf(szRegName,sizeof(szRegName),TEXT("%s_UserDataBasePort"),m_szMainPage);
		RegParamter.SetDWORDValue(szRegName,m_wUserDataBasePort);
...
//加载区域数据
void CInitParamter::LoadAreaData()
{
	//打开注册表
	CRegKey RegParamter;
	TCHAR szRegName[MAX_PATH];
	...
	_snprintf(szRegName,sizeof(szRegName),TEXT("%s_Port"),m_szMainPage);
	dwDataSize=sizeof(dwReadData);
	LONG lErrorCode=RegParamter.QueryValue(szRegName,&dwDataType,&dwReadData,&dwDataSize);
	if (lErrorCode==ERROR_SUCCESS) m_wListenPort=(WORD)dwReadData;
	else m_wListenPort=0;
