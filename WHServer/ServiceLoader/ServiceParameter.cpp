...
void stovs( const std::string& s, const std::string& sp, std::vector<std::string>& sa )
{
	sa.clear();
...
//	((CEdit *)GetDlgItem(IDC_SERVER_RULE))->LimitText(RULE_LEN-1);
...
void CServiceParameter::OnOK()
{
	...
	//房间规则
	GetDlgItemText(IDC_SERVER_RULE,OptionParameter.GameServiceOption.szGameRoomRule,CountArray(OptionParameter.GameServiceOption.szGameRoomRule));
...
//	((CIPAddressCtrl *)GetDlgItem(IDC_VIDEO_ADDR))->GetAddress(OptionParameter.GameServiceOption.dwVideoServer);
	...
	_snprintf(OptionParameter.szDataBaseAddr[0],sizeof(OptionParameter.szDataBaseAddr[0]),TEXT("%d.%d.%d.%d"),
...
bool CServiceParameter::LoadParameter(LPCTSTR pszFileName)
{
	...
	CString szRetTmp;
	LOAD_STRING(pszFileName, "GameServer", "RoomName", szRetTmp);
	lstrcpyn(m_OptionParameter.GameServiceOption.szGameRoomName, szRetTmp, CountArray(m_OptionParameter.GameServiceOption.szGameRoomName));
	szRetTmp = "";
	LOAD_STRING(pszFileName, "GameServer", "ModuleName", szRetTmp);
	lstrcpyn(m_OptionParameter.szModuleName, szRetTmp, CountArray(m_OptionParameter.szModuleName));
	szRetTmp = "";
	LOAD_STRING(pszFileName, "GameServer", "Rule", szRetTmp);
	lstrcpyn(m_OptionParameter.GameServiceOption.szGameRoomRule, szRetTmp, CountArray(m_OptionParameter.GameServiceOption.szGameRoomRule));
	szRetTmp = "";
	LOAD_STRING(pszFileName, "GameServer", "TreasureDBName", szRetTmp);
	lstrcpyn(m_OptionParameter.szDataBaseName, szRetTmp, CountArray(m_OptionParameter.szDataBaseName));
	szRetTmp = "";
	LOAD_STRING(pszFileName, "GameServer", "PrivateRule", szRetTmp);
	lstrcpyn(m_OptionParameter.GameServiceOption.szPrivateRule, szRetTmp, CountArray(m_OptionParameter.GameServiceOption.szPrivateRule));

	
	szRetTmp = "";
	LOAD_STRING(pszFileName, "GameServer", "TreasureDBIP", szRetTmp);
	std::vector<std::string> vecDBIP;
	stovs(szRetTmp.GetBuffer(), ",", vecDBIP);
	for (int i=0; i<vecDBIP.size() && i<MAX_TREASURE_DB; i++)
	{
		lstrcpyn(m_OptionParameter.szDataBaseAddr[i], vecDBIP[i].c_str(), CountArray(m_OptionParameter.szDataBaseAddr[i]));
		m_OptionParameter.m_wTreasureDBCount = i+1;
	}

	szRetTmp = "";
	LOAD_STRING(pszFileName, "GameServer", "IP", szRetTmp);
	m_OptionParameter.GameServiceOption.dwServerAddr=inet_addr(szRetTmp);
	lstrcpyn(m_OptionParameter.GameServiceOption.szServerAddr, szRetTmp, CountArray(m_OptionParameter.GameServiceOption.szServerAddr));
			
	m_OptionParameter.GameServiceOption.wServerID = LOAD_INT(pszFileName, "GameServer", "ID");
	m_OptionParameter.GameServiceOption.wServerPort = m_OptionParameter.GameServiceOption.wServerID;
	m_OptionParameter.GameServiceOption.wSortID = LOAD_INT(pszFileName, "GameServer", "Order");
	m_OptionParameter.GameServiceOption.wKindID = LOAD_INT(pszFileName, "GameServer", "Kind");
	m_OptionParameter.GameServiceOption.wMaxConnect = LOAD_INT(pszFileName, "GameServer", "MaxConnect");
	m_OptionParameter.GameServiceOption.wTableCount = LOAD_INT(pszFileName, "GameServer", "Table");
	m_OptionParameter.GameServiceOption.lCellScore = LOAD_INT(pszFileName, "GameServer", "CellScore");
	m_OptionParameter.GameServiceOption.lMaxScore = LOAD_INT(pszFileName, "GameServer", "MaxScore");
	m_OptionParameter.GameServiceOption.lLessScore = LOAD_INT(pszFileName, "GameServer", "LessScore");
	m_OptionParameter.GameServiceOption.lRestrictScore = LOAD_INT(pszFileName, "GameServer", "RestrictScore");
	m_OptionParameter.GameServiceOption.wRevenue = LOAD_INT(pszFileName, "GameServer", "Revenue");
	m_OptionParameter.GameServiceOption.cbHideUserInfo = LOAD_INT(pszFileName, "GameServer", "Hide");
	m_OptionParameter.GameServiceOption.wServerType = LOAD_INT(pszFileName, "GameServer", "ServerType");
	m_OptionParameter.GameServiceOption.wPrivateType = LOAD_INT(pszFileName, "GameServer", "PrivateType");
...
bool CServiceParameter::LoadGameServiceModule(LPCTSTR pszModuleName)
{
	...
	{
		AfxMessageBox(TEXT("AfxLoadLibrary"),MB_ICONINFORMATION);
		FreeGameServiceModule();
		return false;
	}
	...
	if (CreateFunc==NULL)
	{
		AfxMessageBox(TEXT("GetProcAddress"),MB_ICONINFORMATION);
		FreeGameServiceModule();
		return false;
	}
	...
	{
		AfxMessageBox(TEXT("CreateFunc"),MB_ICONINFORMATION);
		FreeGameServiceModule();
		return false;
	}
...
void CServiceParameter::UpdateOptionParameter()
{
	...
	//房间规则
	if (m_OptionParameter.GameServiceOption.szGameRoomRule[0]==0) 
	{
		TCHAR szServerRule[RULE_LEN]=TEXT("");
		SetDlgItemText(IDC_SERVER_RULE,szServerRule);
	}
	else SetDlgItemText(IDC_SERVER_RULE,m_OptionParameter.GameServiceOption.szGameRoomRule);
...
//		pServerAddr->SetWindowText(szServerAddr);
		pServerAddr->SetWindowText(m_OptionParameter.GameServiceOption.szServerAddr);
	}
	...
	//数据库地址
	DWORD dwDataBaseAddr=ntohl(inet_addr(m_OptionParameter.szDataBaseAddr[0]));
