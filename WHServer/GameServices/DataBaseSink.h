...
#define DBR_GR_START_FINISH				20								//房间启动完成
...
//写游戏记录
struct DBR_GR_WriteGameResultDetails
{
	//用户属性
	DWORD								dwUserID;						//用户
	DWORD								dwClientIP;						//连接地址
...
protected:
	...
	std::vector<tagDataBaseInfo>		* m_pGameScoreDBInfoVec;			//连接信息
...
protected:
	...
	std::vector<CDataBaseAide>			m_GameScoreDBAideVec;			//积分数据库
	CDataBaseHelper						m_AccountsDBModule;				//用户数据库
	CDataBaseHelper						m_GameScoreDBModule;			//积分数据库
	std::vector<IDataBase*>				m_pGameScoreDBModuleVec;		//积分数据库
...
private:
	...
	//房间启动完成
	bool OnRoomStartFinish(DWORD dwContextID, VOID * pData, WORD wDataSize);
...
protected:
	...
	//写游戏积分日志存储过程
	LONG SPGameScoreRecord(DBR_GR_WriteGameResultDetails * pGameScoreRecord);
