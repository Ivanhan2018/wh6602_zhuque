...
static void stovs( const std::string& s, const std::string& sp, std::vector<std::string>& sa )
{
	sa.clear();
	...
	while( true )
	{
		...
		std::string sub = tmps.substr( idx, szsp );
		if( sub == sp )
		{
			std::string item = tmps.substr( 0, idx );
			sa.push_back( item );
			start = 0;
...
bool __cdecl CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	{
		//为每个桌子生成独立的配置参数
		TableFrameParameter.pGameServiceOption = new tagGameServiceOption(*m_pGameServiceOption);
	...
	//连接中心服务器
	for (int i=0; i<m_pITCPSocketCorrespondVec.size(); i++)
	{
		m_pITCPSocketCorrespondVec[i]->Connect(ntohl(m_dwCenterServer[i]),m_wCenterPort[i]);
...
//	m_pITimerEngine->SetTimer(IDI_DISTRIBUTE_ANDROID,TIME_DISTRIBUTE_ANDROID*1000L,(DWORD)(-1),NULL);
	...
	//房间启动完成，通知数据库
	m_pIDataBaseEngine->PostDataBaseRequest(DBR_GR_START_FINISH, 0, m_pGameServiceOption, sizeof(*m_pGameServiceOption));
...
bool __cdecl CAttemperEngineSink::OnAttemperEngineStop(IUnknownEx * pIUnknownEx)
{
	...
	for (int i=0; i<m_pITCPSocketCorrespondVec.size(); i++)
	{
		m_pITCPSocketCorrespondVec[i]->SendData(MDM_CS_SERVER_MANAGER,SUB_CS_UNREG_GAME_SERVER,&UnRegGameServer,sizeof(UnRegGameServer));
	}
...
bool __cdecl CAttemperEngineSink::SendUserScore(IServerUserItem * pIServerUserItem)
{
	...
	//by leon//检查一下日志是否需要写库
	vector<tagGameResultDetails>* pGameLog = (vector<tagGameResultDetails>*)pIServerUserItem->GetGameResultDetails();
	if (pGameLog == NULL) return true;
	for (int i=0; i<pGameLog->size(); i++)
	{
		//写库
		DBR_GR_WriteGameResultDetails gameDetails;
		ZeroMemory(&gameDetails,sizeof(gameDetails));

		//设置变量
		gameDetails.dwUserID = pGameLog->at(i).dwUserID;
		gameDetails.dwClientIP = pGameLog->at(i).dwClientIP;
		gameDetails.lResult = pGameLog->at(i).lResult;
		gameDetails.lRevenue = pGameLog->at(i).lRevenue;
		gameDetails.lScore = pGameLog->at(i).lScore;
		gameDetails.lTotalScore = pGameLog->at(i).lTotalScore;
		gameDetails.lTime = pGameLog->at(i).lTime;
		gameDetails.lAward = pGameLog->at(i).lAward;
		gameDetails.lTableID = pGameLog->at(i).lTableID;

		//投递请求
		m_pIDataBaseEngine->PostDataBaseRequest(DBR_GR_GAME_SCORE_RECORD,0,&gameDetails,sizeof(DBR_GR_WriteGameResultDetails));
	}
	pGameLog->clear();
...
bool __cdecl CAttemperEngineSink::SendUserStatus(IServerUserItem * pIServerUserItem)
{
	...
	if (pIServerUserItem->GetTableID() != INVALID_TABLE)
	{
		...
		{
			IServerUserItem * pIServerUserItemOfTable=pTableFrame->GetServerUserItem(j);
			if (pIServerUserItemOfTable==NULL) continue;
			if(pIServerUserItemOfTable->IsAndroidUser()) continue ;

			SendData(pIServerUserItemOfTable,MDM_GR_USER,SUB_GR_USER_STATUS,&UserStatus,sizeof(UserStatus));
		}
	}
	else
	{
...
//删除用户
bool __cdecl CAttemperEngineSink::RemoveUserItem(IServerUserItem * pIServerUserItem)
{
	...
	if (pIServerUserItem->GetTableID()!=INVALID_TABLE) return false;
	if (pIServerUserItem->GetChairID()!=INVALID_CHAIR) return false;

	//记下桌子的编号
	WORD nTableID = pIServerUserItem->GetTableID();
...
//输出信息
void __cdecl CAttemperEngineSink::ExportInformation(LPCTSTR pszString, int TraceLevel)
{
	CTraceService::TraceString(pszString, (enTraceLevel)TraceLevel);
	return;
}
...
//	WORD wSendSize=sizeof(LogonError)-sizeof(LogonError.szErrorDescribe)+wDescribeSize;
	WORD wSendSize = bos.length();
	if (LOWORD(dwSocketID)>=INDEX_ANDROID)
	{
		m_AndroidUserManager.SendDataToClient(dwSocketID,MDM_GR_LOGON,SUB_GR_LOGON_ERROR,buf,wSendSize);
	}
	else m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GR_LOGON,SUB_GR_LOGON_ERROR,buf,wSendSize);
...
bool CAttemperEngineSink::SendRoomMessage(DWORD dwSocketID, LPCTSTR lpszMessage, WORD wMessageType)
{
	...
	{
		m_pITCPNetworkEngine->SendDataBatch(MDM_GR_SYSTEM,SUB_GR_MESSAGE,cbBuffer,wSendSize);
		m_AndroidUserManager.SendDataToClient(MDM_GR_SYSTEM,SUB_GR_MESSAGE,cbBuffer,wSendSize);
	}
	else
	{
		if (LOWORD(dwSocketID)>=INDEX_ANDROID)
		{
			m_AndroidUserManager.SendDataToClient(dwSocketID,MDM_GR_SYSTEM,SUB_GR_MESSAGE,cbBuffer,wSendSize);
		}
		else m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GR_SYSTEM,SUB_GR_MESSAGE,cbBuffer,wSendSize);
	}
...
bool __cdecl CAttemperEngineSink::OnEventTimer(DWORD dwTimerID, WPARAM wBindParam)
{
	...
	{
		if (dwTimerID >= IDI_CONNECT_CENTER_SERVER_V2 && dwTimerID<=IDI_ATTEMPER_CONCLUDE)
		{
			//发起连接
		//	m_pITCPSocketCorrespond->Connect(ntohl(m_dwCenterServer),PORT_CENTER_SERVER);
			if (wBindParam >= m_pITCPSocketCorrespondVec.size() || wBindParam < 0) return true;
			if (m_pITCPSocketCorrespondVec[wBindParam])
			m_pITCPSocketCorrespondVec[wBindParam]->Connect(ntohl(m_dwCenterServer[wBindParam]),m_wCenterPort[wBindParam]);

			//错误提示
			char buf[128];
			memset(buf, 0, 128);
			sprintf(buf, "正在尝试重新连接中心服务器,编号(%d)",  (int)wBindParam);
		...
		case IDI_UPDATE_ONLINE_COUNT:		//更新人数
			{
				...
				for (int i=0; i<m_pITCPSocketCorrespondVec.size(); i++)
				{
					m_pITCPSocketCorrespondVec[i]->SendData(MDM_CS_SERVER_MANAGER,SUB_CS_SERVER_ONLINE_COUNT,
						&ServerOnLineCount,sizeof(ServerOnLineCount));
				}
				return true;
			}
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	if (pConnectItemInfo->pIServerUserItem!=NULL) return false;

	//消息处理
	DBR_GR_LogonSuccess * pLogonSuccess=(DBR_GR_LogonSuccess *)pData;
	if (pLogonSuccess==NULL) return false;
	...
	lstrcpyn(ServerUserData.szFaceUrl,pLogonSuccess->szFaceUrl,CountArray(ServerUserData.szFaceUrl));
	...
	//广播用户信息
//by leon	SendUserItem(pConnectItemInfo->pIServerUserItem,INVALID_WORD);

	//发送消息
//by leon	SendRoomMessage(dwContextID,TEXT("亲爱的用户，欢迎你多提宝贵建议！"),SMT_INFO);
...
bool CAttemperEngineSink::OnDBLogonError(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	if (pLogonError==NULL) return true;
	...
	{
		if (m_pITCPNetworkEngine)m_pITCPNetworkEngine->ShutDownSocket(dwContextID);
	}
...
bool CAttemperEngineSink::OnDBUserRightResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	if (pUserRightResult == NULL) return false;
	...
	if (pSendUserData == NULL) return false;
	...
	if (pUserRightResult->bGameSuccess || pUserRightResult->bAccountsSuccess)
	{
		...
		while (true)
		{
			...
			if (pIServerUserItemSend->GetUserData()==NULL) break;
...
bool CAttemperEngineSink::OnDBLoadProperty(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	if (pPropertyAttribute == NULL) return false;
...
bool CAttemperEngineSink::OnDBLoadFlower(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	if (pFlowerAttribute == NULL) return false;
...
bool CAttemperEngineSink::OnDBUserProperty(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	if (pUserProperty == NULL) return false;
...
bool CAttemperEngineSink::OnEventTCPNetworkShut(DWORD dwClientIP, DWORD dwActiveTime, DWORD dwSocketID)
{
	...
	if (pIServerUserItem!=NULL)
	{
		...
		if (pIServerUserItem->GetTableID()!=INVALID_TABLE)
		{
			//变量定义
			CTableFrame * pTableFrame=(m_pTableFrame+pIServerUserItem->GetTableID());
			if (pTableFrame==NULL) return true;
...
{
	if (m_pGameServiceOption == NULL) return false;
	if (m_pITimerEngine == NULL) return false;

	//错误判断
	if (nErrorCode!=0)
	{
		//设置时间
		if (m_pITimerEngine)m_pITimerEngine->SetTimer(IDI_CONNECT_CENTER_SERVER_V2+wServiceID,TIME_RECONNECT,1,wServiceID);
	...
	lstrcpyn(RegGameServer.GameServer.szServerAddr,m_pGameServiceOption->szServerAddr,CountArray(RegGameServer.GameServer.szServerAddr));

	char szRule[128];
	memset(szRule, 0, 128);
	sprintf(szRule, "%ld|%ld|%d|%s|%ld", m_pGameServiceOption->lLessScore, 
									 m_pGameServiceOption->lMaxScore, 
									 m_pGameServiceOption->wPrivateType,
									 m_pGameServiceOption->szPrivateRule,
									 m_pGameServiceOption->lCellScore);
	lstrcpyn(RegGameServer.GameServer.szRule, szRule, sizeof(szRule));

	for (int i=0; i<m_pITCPSocketCorrespondVec.size(); i++)
	{
		m_pITCPSocketCorrespondVec[i]->SendData(MDM_CS_SERVER_MANAGER,SUB_CS_REG_GAME_SERVER,&RegGameServer,sizeof(RegGameServer));
	}
...
bool __cdecl CAttemperEngineSink::OnEventTCPSocketRead(WORD wServiceID, CMD_Command Command, VOID * pData, WORD wDataSize)
{
	...
	case MDM_CS_SERVER_MANAGER:  //房间管理 
		{
			return OnCenterMainServerManager(Command,pData,wDataSize, wServiceID);
		}
	}
	return true;
}
...
{
	if (m_pITimerEngine == NULL) return true;

	//重新连接
	m_pITimerEngine->KillTimer(IDI_UPDATE_SERVER_LIST);
	m_pITimerEngine->KillTimer(IDI_UPDATE_ONLINE_COUNT);
	m_pITimerEngine->SetTimer(IDI_CONNECT_CENTER_SERVER_V2+wServiceID,TIME_RECONNECT,1,wServiceID);
...
//			if (wDataSize<sizeof(CMD_GR_LogonByUserID)) return false;
			...
			//处理消息
			CMD_GR_LogonByUserID * pLogonByUserID=&logonByUserID;
			...
			if (pIServerUserItem!=NULL)
			{
				...
				if (LOWORD(dwSocketID)<INDEX_ANDROID)
				{
					if (m_pITCPNetworkEngine)m_pITCPNetworkEngine->AllowBatchSend(dwSocketID,true);
				}
			...
			if (pIServerUserItem!=NULL)
			{
				...
				if (lstrcmp(pIServerUserItem->GetPassword(),pLogonByUserID->szPassWord)!=0)
				{
					...
					{
						if (m_pITCPNetworkEngine)m_pITCPNetworkEngine->ShutDownSocket(dwSocketID);
					}
...
bool CAttemperEngineSink::OnSocketMainUser(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	{
/*
	case SUB_GR_USER_STATUS_REQ:		//查询我的状态
		{
			//获取用户
			IServerUserItem * pIServerUserItem=GetServerUserItem(LOWORD(dwSocketID));
			if (pIServerUserItem==NULL) return false;
			SendData(pIServerUserItem, MDM_GR_USER, SUB_GR_USER_STATUS_REP, NULL, 0);
			return true;
		}
*/
	case SUB_GR_USER_SIT_REQ:		//坐下桌子
	case SUB_GR_USER_SIT_REQ_V2:		//坐下桌子
	case SUB_GR_USER_LOOKON_REQ:	//旁观桌子
		{
			...
			*/
			...
			WORD wTheChairID = INVALID_CHAIR;
			WORD wTheTableID = INVALID_TABLE;
			std::string strThePassword;
			if (wSubCmdID == SUB_GR_USER_SIT_REQ)
			{
				...
				wTheChairID = userSitReq.wChairID;
				wTheTableID = userSitReq.wTableID;
			}
			else if (wSubCmdID == SUB_GR_USER_SIT_REQ_V2)
			{
				CMD_GR_UserSitReq_V2 userSitReq;
				bistream bis;
				bis.attach((char*)pData, wDataSize);
				bis >> userSitReq;
				...
				wTheChairID = userSitReq.wChairID;
				wTheTableID = userSitReq.wTableID;
				strThePassword = userSitReq.szTablePass;
			}

			//判断位置
			if ((wTheChairID>=m_pGameServiceAttrib->wChairCount)&&(wTheChairID!=INVALID_CHAIR)) return false;
			if ((wTheTableID>=m_pGameServiceOption->wTableCount)&&(wTheTableID!=INVALID_TABLE)) return false;
			...
			//随机位置
			if (  (wSubCmdID==SUB_GR_USER_SIT_REQ)
				&&(m_pGameServiceOption->cbHideUserInfo==TRUE)
				&& wTheTableID == INVALID_TABLE)
			{
				WORD wOldTableID = pUserData->wTableID;
				//离开位置
				...
				for (INT_PTR i=0;i<m_pGameServiceOption->wTableCount;i++)
				{
					...
					//获取桌子
					CTableFrame * pTableFrame=(m_pTableFrame+i);
			//by leon不根据游戏是否开始了判断，而是以座位判断		if (pTableFrame->IsGameStarted()==true) continue;

					//by leon 根据FaceID=9999来区分特殊帐号,桌上最多2个9999
					if (pUserData->wFaceID == 9999)
					{
						int iCountSpecial = 0;
						for (int k=0; k<pTableFrame->GetChairCount(); k++)
						{
							IServerUserItem* useritem = pTableFrame->GetServerUserItem(k);
							if (useritem && useritem->GetUserData() && useritem->GetUserData()->wFaceID == 9999)
							{
								iCountSpecial++;
							}
						}
						if (iCountSpecial >= 2)continue;  //此桌不能坐
					}
					...
					if (wChairID!=INVALID_CHAIR)
					{
						...
						for (WORD j=0;j<pTableFrame->GetChairCount();j++)
						{
							...
							SendUserItem(pIServerUserItemOfTable,dwSocketID);

							//发送数据
							WORD wIndex=pIServerUserItemOfTable->GetUserIndex();
							if (wIndex!=INVALID_WORD)
							{
								tagConnectItemInfo * pConnectItemInfo=GetBindParameter(wIndex);
								if (wIndex<INDEX_ANDROID)
								{
									DWORD dwID=pConnectItemInfo->dwSocketID;
									//把我的信息发给同桌玩家
									SendUserItem(pIServerUserItem,  dwID);
								}
							}
						...
						//用户状态
						if (pTableFrame->PerformSitDownAction(wChairID,pIServerUserItem,NULL)==false) return false;
			...
			//处理过虑
			if ((pUserData->wTableID==wTheTableID)&&(pUserData->wChairID==wTheChairID))
			{
			...
			//by leon 只有自建房进入时，才会指定桌号，所以要校验一下
			if (wTheTableID != INVALID_TABLE && wTheTableID < m_pGameServiceOption->wTableCount)
			{
				CTableFrame * pTableFrame=(m_pTableFrame+wTheTableID);
				if ( m_mapSelf.find(wTheTableID) == m_mapSelf.end())
				{
					//构造数据
					CMD_GR_SitFailed SitFailed;
					memset(&SitFailed,0,sizeof(SitFailed));
					_snprintf(SitFailed.szFailedDescribe,sizeof(SitFailed.szFailedDescribe),TEXT("该私人房不存在！"));
				...
				if ( pTableFrame->GetUserCount() == pTableFrame->GetChairCount())
				{
					//构造数据
					CMD_GR_SitFailed SitFailed;
					memset(&SitFailed,0,sizeof(SitFailed));
					_snprintf(SitFailed.szFailedDescribe,sizeof(SitFailed.szFailedDescribe),TEXT("该私人房已满！"));
			...
			//坐下位置
			CTableFrame * pTableFrame=(m_pTableFrame+wTheTableID);
			if (wSubCmdID==SUB_GR_USER_SIT_REQ || wSubCmdID==SUB_GR_USER_SIT_REQ_V2)	
			{
				//坐下处理
				WORD wChairID = INVALID_CHAIR;
				if (wTheChairID == INVALID_CHAIR)
				{
					wChairID=pTableFrame->GetNullChairID();
					//这里一定能取到，因为前面已经校验过人数了
				}
				...
				//真正坐下
				pTableFrame->PerformSitDownAction(wChairID,pIServerUserItem,strThePassword.c_str());
			}
			else
			{
				pTableFrame->PerformLookonAction(wTheChairID,pIServerUserItem,strThePassword.c_str());
			}
...
//			if (wDataSize!=(sizeof(CMD_GR_UserChat)-sizeof(pUserChat->szChatMessage)+pUserChat->wChatLength)) return false;
		...
		case SUB_GR_USER_YY_SEQ:
		{
			CMD_GR_UserYYInfo info;
			bistream bis;
			bis.attach((char*)pData, wDataSize);
			bis >> info;
			...
			CMD_GR_UserYYREPInfo rep; 
			rep.dwUserID = info.dwUserID;
			rep.nLen = info.nLen;
			memset(rep.szExtInfo, 0 , sizeof(rep.szExtInfo));
			memcpy(rep.szExtInfo, info.szExtInfo, info.nLen);
			...
			if (pIServerUserItem->GetTableID() != INVALID_TABLE)
			{
				...
				for (WORD j=0;j<pTableFrame->GetChairCount();j++)
				{
					...
					if (pIServerUserItemOfTable==NULL) continue;
					if(pIServerUserItemOfTable->IsAndroidUser()) continue ;
					SendData(pIServerUserItemOfTable,MDM_GR_USER,SUB_GR_USER_YY_REP,cbBuffer,bos.length() );
				}
			}
			else
			{
				SendData(pIServerUserItem,MDM_GR_USER,SUB_GR_USER_YY_REP,cbBuffer,bos.length() );
			}
...
//	tagUserInfoHead  userInfoHead;
	...
	memcpy(pUserInfoHead->szName, pUserData->szAccounts, sizeof(pUserData->szAccounts));
...
//	bos << userInfoHead;
	...
	//WORD wSendLength = bos.length();
	WORD wSendLength = sizeof(tagUserInfoHead);
	...
	SendUserItem_V2(pIServerUserItem,dwSocketID);

	return true;
}

//发送用户
bool CAttemperEngineSink::SendUserItem_V2(IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	//效验参数
	ASSERT(pIServerUserItem!=NULL);
...
//	tagUserInfoHead  userInfoHead;
	tagUserInfoHead_V2 * pUserInfoHead = (tagUserInfoHead_V2*)cbBuffer;
	tagServerUserData * pUserData=pIServerUserItem->GetUserData();

	//填写数据
	memset(pUserInfoHead,0,sizeof(tagUserInfoHead_V2));
	pUserInfoHead->wFaceID=pUserData->wFaceID;
	pUserInfoHead->dwCustomFaceVer=pUserData->dwCustomFaceVer;
	pUserInfoHead->wTableID=pUserData->wTableID;
	pUserInfoHead->wChairID=pUserData->wChairID;
	...
	pUserInfoHead->UserScoreInfo.lScore=pUserData->UserScoreInfo.lScore+pUserData->lStorageScore;
	memcpy(pUserInfoHead->szName, pUserData->szAccounts, sizeof(pUserData->szAccounts));
	memcpy(pUserInfoHead->szUnderWrite, pUserData->szUnderWrite, sizeof(pUserData->szUnderWrite));
	memcpy(pUserInfoHead->szFaceUrl, pUserData->szFaceUrl, sizeof(pUserData->szFaceUrl));
...
//	bos << userInfoHead;
	...
	//WORD wSendLength=sizeof(tagUserInfoHead)+SendPacket.GetDataSize();
	//WORD wSendLength = bos.length();
	WORD wSendLength = sizeof(tagUserInfoHead_V2);

	if (LOWORD(dwSocketID)!=INVALID_WORD) 
	{
		if (LOWORD(dwSocketID)>=INDEX_ANDROID)
		{
			m_AndroidUserManager.SendDataToClient(dwSocketID,MDM_GR_USER,SUB_GR_USER_COME_V2,cbBuffer,wSendLength);
		}
		else m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GR_USER,SUB_GR_USER_COME_V2,cbBuffer,wSendLength);
	}
	else m_pITCPNetworkEngine->SendDataBatch(MDM_GR_USER,SUB_GR_USER_COME_V2,cbBuffer,wSendLength);
...
bool CAttemperEngineSink::SendGameServerInfo(IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	...
	if (pIServerUserItem->GetTableID() != INVALID_TABLE)
	{
		WORD wTableID = pIServerUserItem->GetTableID();
		WORD wChairID = pIServerUserItem->GetChairID();
		CTableFrame * pTableFrame=(m_pTableFrame+wTableID);
		if (pTableFrame)
		{
			//再发同桌的其他玩家的信息
			for (WORD j=0;j<pTableFrame->GetChairCount();j++)
...
bool CAttemperEngineSink::OnCenterMainControlCommand(CMD_Command Command, VOID * pData, WORD wDataSize)
{
	...
	case SUB_CS_CONTROL_COMMAND_REQ:		//请求命令
		{
			//效验参数
			ASSERT(wDataSize==sizeof(CMD_CS_ControlCommandReq));
			if (wDataSize!=sizeof(CMD_CS_ControlCommandReq)) return false;

			//消息处理
			CMD_CS_ControlCommandReq* req = (CMD_CS_ControlCommandReq*)pData;
			if (req == NULL)
			{
				CTraceService::TraceString("命令字消息体不正确 SUB_CS_CONTROL_COMMAND_REQ", TraceLevel_Warning);
				return true;
			}

			if (req->wCommandID == 1)  //管理员踢人
			{
				//查找用户
				IServerUserItem* pUser = m_ServerUserManager.SearchOnLineUser(req->dwUserID);
				if (pUser)
				{
					//用户在本房间
					char buf[128];
					memset(buf, 0, 128);
					sprintf(buf, "在本房间找到用户 %u", req->dwUserID);
					CTraceService::TraceString(buf, TraceLevel_Normal);

					//给该用户发送踢出消息，让用户在本局游戏结束后主动退出
				//	TCHAR szDescribe[128]=TEXT("");
				//	_snprintf(szDescribe,sizeof(szDescribe),TEXT("您被管理员踢出游戏桌！"));
				//	SendGameMessage(pUser,szDescribe,SMT_EJECT|SMT_INFO|SMT_CLOSE_GAME);	
					
					LPCTSTR szMessage=TEXT("抱歉，您将被管理员踢出房间，若有任何疑问，请联系游戏客服！");
					SendRoomMessage(pUser,szMessage,SMT_INFO|SMT_EJECT|SMT_INTERMIT_LINE);

					//回复消息
					CMD_CS_ControlCommandRep rep;
					rep.dwServerID = m_pGameServiceOption->wServerID;
					rep.wCommandID = req->wCommandID;
					rep.dwUserID = pUser->GetUserID();
					rep.wCode = 0;
					for (int i=0; i<m_pITCPSocketCorrespondVec.size(); i++)
					{
						m_pITCPSocketCorrespondVec[i]->SendData(MDM_CS_CONTROL_COMMAND,
							SUB_CS_CONTROL_COMMAND_REP,
							&rep,
							sizeof(CMD_CS_ControlCommandRep));
					}
				}
			}
			else if (req->wCommandID == 2) //房间进入维护期，禁止新玩家登录
			{
				if (req->dwServerID == m_pGameServiceOption->wServerID)
				{
					CTraceService::TraceString("接收到控制命令：房间进入维护期", TraceLevel_Warning);
					m_bShallClose = true;
	...
	default:
		{
			char buf[128];
			memset(buf, 0, 128);
			sprintf(buf, "未知命令字 %d", Command.wSubCmdID);
...
//房间管理处理
bool CAttemperEngineSink::OnCenterMainServerManager(CMD_Command Command, VOID * pData, WORD wDataSize, WORD wServiceID)
{
	switch (Command.wSubCmdID)
	{
		case SUB_CS_CREATE_SELF_REQ:		//请求创建房间
		{
			//效验参数
			...
			//消息处理
			CMD_CS_CreateSelf* req = (CMD_CS_CreateSelf*)pData;
			if (req == NULL)
			{
				CTraceService::TraceString("命令字消息体不正确 SUB_CS_CREATE_SELF_REQ", TraceLevel_Warning);
				return true;
			}

			CMD_CS_CreateSelfRep rep;
			memset(&rep, 0, sizeof(CMD_CS_CreateSelfRep));
			rep.dwSocketID = req->dwSocketID;
			rep.dwSocketIDLogin = req->dwSocketIDLogin;
			rep.nServerID = req->nServerID;
			rep.nCode = 1;   //默认为失败

			//解析规则
			DWORD lCellScoreFromRule = 0;
			DWORD lLessScoreFromRule = 0;
			DWORD lMaxScoreFromRule = 0;
			DWORD lUserIDFromRule = 0;
			std::vector<std::string> vecParams;
			stovs(req->szRule, ",", vecParams);
			for (int k=0; k<vecParams.size(); k++)
			{
				std::vector<std::string> vecKV;
				stovs(vecParams[k], ":", vecKV);
				if (vecKV.size() == 2)
				{
					if (vecKV[0] == "a")  //单元积分
					{
						lCellScoreFromRule = atoi(vecKV[1].c_str());
					}
					else if (vecKV[0] == "m") //最小财富
					{
						lLessScoreFromRule = atoi(vecKV[1].c_str());
					}
					else if (vecKV[0] == "x") //最大财富
					{
						lMaxScoreFromRule = atoi(vecKV[1].c_str());
					}
					else if (vecKV[0] == "u") //创建者ID
					{
						lUserIDFromRule = atoi(vecKV[1].c_str());
					}
				}
			}

			//如果没有人的自建场，并且时间超过1分钟，就清除掉。自己建的私人场，不考虑时间
			vector<WORD> toDelete;
			for (std::map<WORD, tagTableSelf>::iterator cit = m_mapSelf.begin();
				cit != m_mapSelf.end();
				++cit) 
			{
				if (m_pTableFrame[cit->first].GetUserCount() == 0)
				{
					//是否自己建的
					if (cit->second.m_dwCreateUserID == lUserIDFromRule)
					{
						CTraceService::TraceString(TEXT("找到1个自己建的，要删除"),TraceLevel_Normal);
						toDelete.push_back(cit->first);
					}
					else if (cit->second.m_dwCreateTime + 60 < (DWORD)time(NULL))
					{
						CTraceService::TraceString(TEXT("找到1个超时的"),TraceLevel_Normal);
						toDelete.push_back(cit->first);
					}
				}
			}
			...
			{
				m_mapSelf.erase(toDelete[i]);
			}

			//校验房间名称是否重复
			for (std::map<WORD, tagTableSelf>::const_iterator cit = m_mapSelf.begin();
				cit != m_mapSelf.end();
				++cit)
			{
				if (lstrcmp(cit->second.m_szName,req->szName)==0)
				{
					//重名
					rep.nCode = 3;
					break;
			...
			//处理结果
			if (   rep.nCode == 1
				&& m_mapSelf.size() < m_pGameServiceOption->wTableCount) 
			{
				//桌子没有用完。则遍历，找到第1个没有人的空桌
				for (INT_PTR i=0;i<m_pGameServiceOption->wTableCount;i++)
				{
					//获取桌子
					CTableFrame * pTableFrame=(m_pTableFrame+i);
					if (pTableFrame->GetUserCount() == 0 && m_mapSelf.end() == m_mapSelf.find(i))
					{
						//找到了
						m_mapSelf[i] = tagTableSelf();
						m_mapSelf[i].m_dwCreateTime = (DWORD)time(NULL);
						m_mapSelf[i].m_dwCreateUserID = lUserIDFromRule;
						lstrcpyn(m_mapSelf[i].m_szName, req->szName, sizeof(req->szName));
						lstrcpyn(m_mapSelf[i].m_szRule, req->szRule, sizeof(req->szRule));
						lstrcpyn(m_mapSelf[i].m_szPassword, req->szPassword, sizeof(req->szPassword));
						lstrcpyn(pTableFrame->m_szPassword, req->szPassword, sizeof(req->szPassword));
						if (std::string(req->szPassword).size() > 0)
						{
							pTableFrame->m_bTableLocked = true;
						}
						else
						{
							pTableFrame->m_bTableLocked = false;
						}

						char buf[NAME_LEN];
						memset(buf, 0, NAME_LEN);
						sprintf(buf, "%d%03d", req->nServerID, i+1);
						lstrcpyn(m_mapSelf[i].m_szID, buf, sizeof(m_mapSelf[i].m_szID));

						//重新订制桌子参数
						//lstrcpyn(pTableFrame->m_pGameServiceOption->szGameRoomRule,req->szRule, sizeof(req->szRule));
						//解析规则
						pTableFrame->m_pGameServiceOption->lCellScore = lCellScoreFromRule;
						pTableFrame->m_pGameServiceOption->lLessScore = lLessScoreFromRule;
						pTableFrame->m_pGameServiceOption->lMaxScore = lMaxScoreFromRule;

						//根据底分来生成茶水费比例
						std::vector<std::string> vecParams;
						stovs(pTableFrame->m_pGameServiceOption->szPrivateRule, ",", vecParams);
						for (int k=0; k<vecParams.size(); k++)
						{
							std::vector<std::string> vecKV;
							stovs(vecParams[k], "/", vecKV);
							if (vecKV.size() >= 3)
							{
								if (atoi(vecKV[0].c_str()) == lCellScoreFromRule)
								{
									///税收比例
									pTableFrame->m_pGameServiceOption->wRevenue = atoi(vecKV[2].c_str());
									break;
								}
							}
						}
						...
						rep.nCode = 0;  //成功
						lstrcpyn(rep.szName, m_mapSelf[i].m_szName, sizeof(rep.szName));
						lstrcpyn(rep.szRoomID, m_mapSelf[i].m_szID, sizeof(rep.szRoomID));

						char bufRule[128];
						memset(bufRule, 0, 128);
						sprintf(bufRule, "t:%d|%s", i, m_mapSelf[i].m_szRule);
						lstrcpyn(rep.szRule, bufRule, sizeof(rep.szRule));
						break;
					}
				}
			}

			//根据wServiceID，将消息返回给指定的请求者CenterServer
			if (wServiceID<m_pITCPSocketCorrespondVec.size())
			{
				m_pITCPSocketCorrespondVec[wServiceID]->SendData(MDM_CS_SERVER_MANAGER,
					SUB_CS_CREATE_SELF_REP,
					&rep,
					sizeof(CMD_CS_CreateSelfRep));
			}

			return true;
		}
		case SUB_CS_CHECK_PWD_SELF_REQ:		//请求校验密码
		{
			//效验参数
			...
			//消息处理
			CMD_CS_CheckPwdSelf* req = (CMD_CS_CheckPwdSelf*)pData;
			if (req == NULL)
			{
				CTraceService::TraceString("命令字消息体不正确 SUB_CS_CHECK_PWD_SELF_REQ", TraceLevel_Warning);
				return true;
			}

			CMD_CS_CheckPwdSelfRep rep;
			memcpy(rep.szRoomID, req->szRoomID, sizeof(rep.szRoomID));
			rep.dwSocketID = req->dwSocketID;
			rep.dwSocketIDLogin = req->dwSocketIDLogin;
			rep.nServerID = req->nServerID;
			rep.nCode = 1;   //默认为失败

			std::string toFindID(req->szRoomID);
			for (std::map<WORD, tagTableSelf>::const_iterator cit = m_mapSelf.begin();
				cit != m_mapSelf.end();
				++cit)
			{
				if (toFindID == cit->second.m_szID)
				{
					//找到了

					std::string thePassword(req->szPassword);
					if (thePassword == cit->second.m_szPassword)
					{
						rep.nCode = 0;
					}
					break;
			...
			{
				m_pITCPSocketCorrespondVec[wServiceID]->SendData(MDM_CS_SERVER_MANAGER,
					SUB_CS_CHECK_PWD_SELF_REP,
					&rep,
					sizeof(CMD_CS_CheckPwdSelfRep));
			}

			return true;
		}
		case SUB_CS_QUERY_SELF_REQ:		//请求查询房间
		{
			//效验参数
			...
			//消息处理
			CMD_CS_QuerySelf* req = (CMD_CS_QuerySelf*)pData;
			if (req == NULL)
			{
				CTraceService::TraceString("命令字消息体不正确 SUB_CS_QUERY_SELF_REQ", TraceLevel_Warning);
				return true;
			}

			//如果没有人的自建场，并且时间超过1分钟，就清除掉
			vector<WORD> toDelete;
			for (std::map<WORD, tagTableSelf>::iterator cit = m_mapSelf.begin();
				cit != m_mapSelf.end();
				++cit) 
			{
				if (   m_pTableFrame[cit->first].GetUserCount() == 0
					&& cit->second.m_dwCreateTime + 60 < (DWORD)time(NULL))
				{
					CTraceService::TraceString(TEXT("找到1个超时的"),TraceLevel_Normal);
					toDelete.push_back(cit->first);
				}
			...
			CMD_CS_QuerySelfRep rep;
			memset(&rep, 0, sizeof(CMD_CS_QuerySelfRep));
			rep.dwSocketID = req->dwSocketID;
			rep.dwSocketIDLogin = req->dwSocketIDLogin;
			rep.nServerID = req->nServerID;
			rep.nPage = req->nPage;
			...
			rep.nTotal = m_mapSelf.size();  //总数
			
			//处理结果
			int iCntEveryPage = 5;  //暂定为每次返回10条，szRule长度不能超过640
			int iCnt = 0;
			...
			{
				//校验房间名称是否满足条件
				bool bMatchName = true;
				bool bMatchID = true;
				if (std::string(req->szName).size() > 0)
				{
					if (std::string(cit->second.m_szName).find(req->szName) != std::string::npos)
					{
						//匹配
					}
					else
				...
				if (std::string(req->szRoomID).size() > 0)
				{
					if (std::string(cit->second.m_szID).find(req->szRoomID) != std::string::npos)
					{
						//匹配
					}
					else
				...
				//获取桌子
				CTableFrame * pTableFrame=(m_pTableFrame + cit->first);
				if (pTableFrame == NULL)
				{
					//跳过失效的桌子
					continue;
				...
				//房间无人的，要跳过
				if (pTableFrame->GetUserCount() == 0)
				{
					continue;
				}

				iCnt++;
				if (iCnt <= rep.nPage * iCntEveryPage)
				{
					continue;
				}
				else if (iCnt > (rep.nPage+1) * iCntEveryPage)
				{
					//满了
					break;
				}
				...
				//添加记录
				rep.nPage = req->nPage;
				rep.nCount = iCnt - rep.nPage * iCntEveryPage;
				lstrcpyn(rep.szName[rep.nCount-1], cit->second.m_szName, sizeof(cit->second.m_szName));

				char buf[192];
				memset(buf, 0, 192);
				sprintf(buf, "id:%s,t:%d,c:%d,k:%d,%s", cit->second.m_szID, cit->first,pTableFrame->GetUserCount(), 
								pTableFrame->IsTableLocked()?1:0,
								cit->second.m_szRule); 
				if (strExtend.size() > 0)
				{
					strExtend += "|";
			...
			if (strExtend.size() > 0 && strExtend.size() < sizeof(rep.szExtend))
			{
				//有效
				rep.nCode = 0;
				lstrcpyn(rep.szExtend, strExtend.c_str(), strExtend.size()+1);
			}
			...
			{
				m_pITCPSocketCorrespondVec[wServiceID]->SendData(MDM_CS_SERVER_MANAGER,
					SUB_CS_QUERY_SELF_REP,
					&rep,
					sizeof(CMD_CS_QuerySelfRep));
			}
	...
	case SUB_CS_SPEAKER_SEND_MSG_REQ:		//从中心服务器转过来的喇叭消息
		{
			...
			//转发给所有在线用户
			CMD_GR_SpeakerMsgRep repGS;
			repGS.nLen = repCS->nLen;
			repGS.nType = repCS->nType;
			memcpy(repGS.szMsg, repCS->szMsg, repCS->nLen);
			...
			while (true)
			{
				...
				//发送数据
				SendData( pIServerUserItemSend, MDM_GR_USER, SUB_GR_USER_SPEAKER_MSG_REP, cbBuffer,bos.length() );
			}
	...
	default:
		{
			char buf[128];
			memset(buf, 0, 128);
			sprintf(buf, "未知自建场命令字 %d", Command.wSubCmdID);
			CTraceService::TraceString(buf, TraceLevel_Warning);
		}
	}
