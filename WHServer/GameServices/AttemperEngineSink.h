...
struct tagTableSelf
{
	DWORD							m_dwCreateTime;					//刷新时间
	DWORD							m_dwCreateUserID;				//创建者
	TCHAR							m_szName[NAME_LEN];				//桌子名称
	TCHAR							m_szPassword[PASS_LEN];				//桌子密码
	TCHAR							m_szID[NAME_LEN];				//桌子ID
	TCHAR							m_szRule[128];					//桌子规则
};
...
protected:
	...
	//自建场信息
protected:
	std::map<WORD, tagTableSelf>		m_mapSelf;			//记录下已经创建好的自建场
...
protected:
	...
	//配置信息
protected:
	DWORD							m_dwCenterServer[MAX_CENTER_SERVER];					//中心地址
	WORD							m_wCenterPort[MAX_CENTER_SERVER];						//中心地址端口
...
protected:
	...
	vector<ITCPSocketService *>		m_pITCPSocketCorrespondVec;			//协调服务
...
public:
	//删除用户
	virtual bool __cdecl RemoveUserItem(IServerUserItem * pIServerUserItem);
	//删除用户
	virtual bool __cdecl DeleteUserItem(IServerUserItem * pIServerUserItem);
	//输出信息
	virtual void __cdecl ExportInformation(LPCTSTR pszString, int TraceLevel);
...
private:
	...
	//发送用户
	bool SendUserItem_V2(IServerUserItem * pIServerUserItem, DWORD dwSocketID);
	//发送信息
...
private:
	...
	//房间管理处理
	bool OnCenterMainServerManager(CMD_Command Command, VOID * pData, WORD wDataSize, WORD wServiceID);
