...
bool __cdecl CTableFrame::InitTableFrame(WORD wTableID, tagTableFrameParameter * pTableFrameParameter)
{
	...
	m_pGameServiceOption=(tagGameServiceOption*)pTableFrameParameter->pGameServiceOption;
...
bool __cdecl CTableFrame::PerformUserLeave(IServerUserItem * pIServerUserItem)
{
	//效验参数
...
//		}
		...
		m_pIGameServiceFrame->SendUserStatus(pIServerUserItem);  //要用到桌子
...
bool __cdecl CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR szPassword)
{
	...
	{
		wChairID=GetNullChairID();
		if(wChairID==INVALID_CHAIR)
		{
			SendSitFailedPacket(pIServerUserItem,TEXT("您选择的房间已满！"));
			return false;
		}
	}
	...
	else
	{
		...
		if (lLessScore!=0L || lMaxScore>lLessScore)
		{
			if (pUserData->UserScoreInfo.lScore<lLessScore)
			{
				...
				{
					_snprintf(szDescribe,sizeof(szDescribe),TEXT("加入游戏至少需要 %ld 个欢乐豆，您的欢乐豆不够，不能加入！"),lLessScore);
				}
				else
				{
			...
			if (lMaxScore>lLessScore && lMaxScore<pUserData->UserScoreInfo.lScore)
			{
				...
				if (m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
				{
					_snprintf(szDescribe,sizeof(szDescribe),TEXT("您的欢乐豆超过本房间的最大限制额度%ld,请更换游戏房间!"),lMaxScore);
				}
				else
				{
...
bool __cdecl CTableFrame::OnUserReqOffLine(WORD wChairID)
{
	...
	// 三张不限制退出次数
	if (m_pGameServiceOption->wKindID == 961)
	{
		return true;
	}
	//常规模式
...
bool __cdecl CTableFrame::SendTableData(WORD wChairID, WORD wSubCmdID)
{
	...
	if (wChairID==INVALID_CHAIR)
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
	//by leon		if ((m_pIUserItem[i]!=NULL)&&(IsClientReady(m_pIUserItem[i])==true))
			if (m_pIUserItem[i]!=NULL)
			{
...
//写入积分
bool __cdecl CTableFrame::WriteUserScore(WORD wChairID, LONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount, LONG lAward)
{
	//效验参数
	ASSERT(wChairID<m_wChairCount);
	ASSERT(m_pIUserItem[wChairID]!=NULL);
	...
	//写入积分
	WriteUserScore(m_pIUserItem[wChairID],lScore,lRevenue,ScoreKind, lPlayTimeCount, lAward);

	return true;
}

bool __cdecl CTableFrame::DeletGameSceneInfo(IServerUserItem * pIServerUserItem)
{
	//效验参数
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem==NULL) return false;

	PerformUserLeave(pIServerUserItem);

	//发送信息
	m_pIGameServiceFrame->RemoveUserItem(pIServerUserItem);
}

//写入积分
bool __cdecl CTableFrame::WriteUserScore(IServerUserItem * pIServerUserItem, LONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount, LONG lAward)
{
	//效验参数
	ASSERT(pIServerUserItem!=NULL);
	if (pIServerUserItem==NULL) return false;
	...
	//校验数据，不能出现负分
	if ((pIServerUserItem->GetUserScore()->lScore + lScore) < 0)
	{
		//出现负分的情况了
		//最后一个都没有处理成功
		char buf[128];
		memset(buf, 0, 128);
		sprintf(buf, "GSP_GR_WriteGameRecord_V2 负分. id=%ld totalscore=%d score=%ld", pIServerUserItem->GetUserID(), pIServerUserItem->GetUserScore()->lScore, lScore);
...
bool __cdecl CTableFrame::SendGameScene(IServerUserItem * pIServerUserItem, void * pData, WORD wDataSize)
{
	...
	if (m_pGameServiceOption->wServerType==GAME_GENRE_MATCH)
	{
//by leon		_snprintf(szMessage,sizeof(szMessage),TEXT("亲爱的用户，欢迎您来到“%s”比赛游戏房间，祝比赛选手赛出水平，取得好的成绩！"),m_pGameServiceAttrib->szKindName);
//by leon		SendGameMessage(pIServerUserItem,szMessage,SMT_INFO);
	}
	else
	{
//by leon		_snprintf(szMessage,sizeof(szMessage),TEXT("亲爱的用户，欢迎您来到“%s”，欢迎你多提宝贵建议！"),m_pGameServiceAttrib->szKindName);
//by leon		SendGameMessage(pIServerUserItem,szMessage,SMT_INFO);
	}
	...
	{
//by leon		lstrcpyn(szMessage,TEXT("请注意，您设置了允许所有玩家观看您游戏。"),CountArray(szMessage));
//by leon		SendGameMessage(pIServerUserItem,szMessage,SMT_INFO);
	}
...
void CTableFrame::SendSitFailedPacket(IServerUserItem * pIServerUserItem, LPCTSTR pszFailedDescribe)
{
	...
	m_pIGameServiceFrame->SendData(pIServerUserItem,MDM_GR_USER,SUB_GR_SIT_FAILED,&SitFailed,sizeof(SitFailed));
