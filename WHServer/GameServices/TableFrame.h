...
public:
	...
	//游戏房间解锁
	virtual bool __cdecl DeletGameSceneInfo(IServerUserItem * pIServerUserItem);
	//消息接口
public:
	//发送房间消息
...
public:
	//写入积分
	virtual bool __cdecl WriteUserScore(WORD wChairID, LONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount=-1, LONG lAward=0);
	//写入积分
	virtual bool __cdecl WriteUserScore(IServerUserItem * pIServerUserItem, LONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount=-1, LONG lAward=0);
...
public:
	...
	//中途离开动作
	virtual bool __cdecl PerformUserLeave(IServerUserItem * pIServerUserItem);
	//用户事件
public:
	//断线事件
