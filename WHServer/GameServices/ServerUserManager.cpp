...
bool __cdecl CServerUserItem::WriteScore(tagScoreInfo & ScoreInfo, DWORD dwPlayTimeCount)
{
	...
	//by leon写日志库
	tagGameResultDetails GameResult;
	ZeroMemory(&GameResult,sizeof(GameResult));
	GameResult.dwUserID = GetUserID();
	GameResult.dwClientIP = GetClientIP();
	GameResult.lRevenue = ScoreInfo.lRevenue;
	GameResult.lScore = ScoreInfo.lScore;
	GameResult.lTotalScore = m_ServerUserData.UserScoreInfo.lScore;
	GameResult.lResult = ScoreInfo.ScoreKind+1;   //把0跳过
	GameResult.lTime = (LONG)time(NULL);
	GameResult.lAward = ScoreInfo.lAward;  //奖励
	GameResult.lTableID = GetTableID() + 1;  //从1开始

	m_vecGameLog.push_back(GameResult);
