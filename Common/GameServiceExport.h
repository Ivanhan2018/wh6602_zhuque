...
#define IDI_CONNECT_CENTER_SERVER_V2	(IDI_ATTEMPER_START+60)			//连接多中心服务器
...
#define MAX_CENTER_SERVER				32
#define MAX_TREASURE_DB					32
//////////////////////////////////////////////////////////////////////////
//枚举定义
...
struct tagGameServiceParameter
{
	...
	DWORD								dwCenterAddr[MAX_CENTER_SERVER];						//中心地址
	WORD								wCenterPort[MAX_CENTER_SERVER];						//中心地址端口
	WORD								wCenterCount;						//中心服务器个数，不超过MAX_CENTER_SERVER
	tagDataBaseInfo						GameUserDBInfo;						//数据配置
	tagDataBaseInfo						GameScoreDBInfo[MAX_TREASURE_DB];	//数据配置
	WORD								wTreasureDBCount;					//财富数据库个数，不超过MAX_TREASURE_DB
	tagGameServiceOption				GameServiceOption;					//服务配置
};

//游戏记录
struct tagGameResultDetails
{
	//用户属性
	DWORD								dwUserID;						//用户
...
public:
	...
	//待入库的游戏日志信息
	virtual void* __cdecl GetGameResultDetails()=NULL;
...
public:
	...
	//游戏房间解锁
	virtual bool __cdecl DeletGameSceneInfo(IServerUserItem * pIServerUserItem) = NULL;
	//写分接口
public:
	//写入积分
	virtual bool __cdecl WriteUserScore(WORD wChairID, LONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount=-1, LONG lAward=0)=NULL;
	//写入积分
	virtual bool __cdecl WriteUserScore(IServerUserItem * pIServerUserItem, LONG lScore, LONG lRevenue, enScoreKind ScoreKind, LONG lPlayTimeCount=-1, LONG lAward=0)=NULL;
};
...
public:
	...
	//中途离开动作
	virtual bool __cdecl PerformUserLeave(IServerUserItem * pIServerUserItem)= NULL;
...
public:
	...
	//管理接口
public:
	virtual bool __cdecl RemoveUserItem(IServerUserItem * pIServerUserItem)=NULL;
	//删除用户
	virtual bool __cdecl DeleteUserItem(IServerUserItem * pIServerUserItem)=NULL;
	//输出信息
	virtual void __cdecl ExportInformation(LPCTSTR pszString, int TraceLevel)=NULL;
};
