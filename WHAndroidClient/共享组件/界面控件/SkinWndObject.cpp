...
void CSkinWndObject::InitSkinObject()
{
	...
	if (lWindowStyte&WS_SYSMENU)
	{
		...
		if ((lWindowStyte&WS_MAXIMIZEBOX)||(lWindowStyte&WS_MINIMIZEBOX)) 
		{
			...
			{
				m_cbButtonState[BST_MAX]=BUT_EXIST|BUT_NORMAL;
			}
			else 
			{
				m_cbButtonState[BST_MAX]=BUT_EXIST|BUT_DISABLE;
			...
			if (lWindowStyte&WS_MINIMIZEBOX)
			{
...
void CSkinWndObject::DrawSkinView(CDC * pDC)
{
	...
	CaptionImage.Create(ClientRect.Width()-2*m_nXExcursionPos,m_SkinAttribute.m_nCaptionHeigth,32);
	...
	long dwStyle = GetWindowLong(m_pWndHook->m_hWnd,GWL_EXSTYLE);
...
{
	if (!::IsWindow(m_pWndHook->m_hWnd)) return false;
...
bool CSkinWndObject::OnMouseMoveEvent(CPoint ClientPoint)
{
	...
	return true; //2011-3-9 yangyan add 消除鼠标左键事件
