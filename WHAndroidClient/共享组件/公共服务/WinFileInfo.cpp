...
bool CWinFileInfo::OpenWinFile(LPCTSTR lpszFileName)
{
    ...
    if (::GetFileVersionInfo(lpszFileName,dwFileHandle,sizeof(m_cbInfoBuffer),m_cbInfoBuffer)==FALSE) 
	{
		return false;
	}
	...
	//版本信息
    if (::VerQueryValue(m_cbInfoBuffer,TEXT("\\"),(void * *)&m_pFixedFileInfo,&uQuerySize)==FALSE) 
	{
		return false;
	}
	if ((uQuerySize!=sizeof(VS_FIXEDFILEINFO))||(m_pFixedFileInfo==NULL)) 
	{
		return false;
	}
    return true;
