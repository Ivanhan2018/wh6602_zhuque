...
BEGIN_MESSAGE_MAP(CFindUserDlg, CDialog)
	...
	ON_NOTIFY(NM_DBLCLK, IDC_LIST, OnNMDBclickList)
END_MESSAGE_MAP()

//构造函数
CFindUserDlg::CFindUserDlg(CWnd * pParentWnd, UINT uGameType) : CDialog(IDD_FIND_USER)
{
...
void CFindUserDlg::OnNMDBclickList(NMHDR * pNMHDR, LRESULT * pResult)
{
	NMITEMACTIVATE * pListNotify=(NMITEMACTIVATE *)pNMHDR;
	if (-1 != pListNotify->iItem)
	{
		CPoint MousePoint;
		GetCursorPos(&MousePoint);
		CRoomViewItem * pGameRoom=(CRoomViewItem *)m_pParentWnd;
		...
		GetParent()->PostMessage(WM_FLASH_CHAIR, (WPARAM)pUserItem->GetUserData()->dwUserID, 0);
	}
}
...
void CFindUserDlg::UpdateUserItem(IUserItem * pUserItem)
{
	...
	{
		m_UserList.UpdateUserItem(pUserItem);
	}
	return;
}
