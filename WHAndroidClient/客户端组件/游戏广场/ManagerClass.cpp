...
BEGIN_MESSAGE_MAP(CManagerServer, CDialog)//CSkinDialogEx
END_MESSAGE_MAP()

BEGIN_MESSAGE_MAP(CManagerMessage, CDialog)//CSkinDialogEx)
...
BEGIN_MESSAGE_MAP(CManagerSendWarning, CDialog)
	ON_BN_CLICKED(IDC_BT_PRIVIEW, OnPreViewMessage)
	ON_BN_CLICKED(IDC_BT_SEND, OnOK)
...
//构造函数
CManagerSendWarning::CManagerSendWarning() : CDialog(IDD_SEND_WARNING)
{
	//设置变量
	m_dwUserID=0L;
	m_pIUserItem=NULL;
...
void CManagerSendWarning::DoDataExchange(CDataExchange * pDX)
{
	...
	DDX_Control(pDX, IDC_BT_SEND, m_btSend);
	DDX_Control(pDX, IDC_BT_CANCEL, m_btClose);
	DDX_Control(pDX, IDC_BT_PRIVIEW, m_btPreView);
...
{
	//AfxMessageBox(TEXT("CManagerSendWarning::OnOK"));
	//调整用户
...
//构造函数
CManagerMessage::CManagerMessage() :CDialog()//: CSkinDialogEx(IDD_MANAGER_MESSAGE)
{
	//设置接口
	m_pIClientSocket=NULL;
...
//构造函数
CManagerServer::CManagerServer() : CDialog()//: CSkinDialogEx(IDD_MANAGER_SERVER)
{
	//设置接口
	m_pIClientSocket=NULL;
...
void CManagerServer::DoDataExchange(CDataExchange * pDX)
{
	...
	DDX_Control(pDX, IDC_EXITMANEGERWINDOW, m_btExitManagerWindow);
}
...
BOOL CManagerServer::OnInitDialog()
{
	...
	m_btLimitRoomWisper.ShowWindow(SW_HIDE);
	m_btAllowRoomWisper.ShowWindow(SW_HIDE);
	m_btLimitRoomChat.ShowWindow(SW_HIDE);
	m_btAllowRoomChat.ShowWindow(SW_HIDE);
	m_btLimitGameChat.ShowWindow(SW_HIDE);
	m_btAllowGameChat.ShowWindow(SW_HIDE);
	m_btLimitEnterGame.ShowWindow(SW_HIDE);
	m_btAllowEnterGame.ShowWindow(SW_HIDE);
...
BOOL CManagerServer::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case IDC_EXITMANEGERWINDOW:		//关闭管理窗口
		{
			OnCancel();
			return TRUE;
		}
	...
	if (OptionServer.cbOptionFlags!=0)
	{
		...
		//====Author(cxf);DateTime<2012-03-15>;Description<即将关闭的同时禁止进入游戏：恢复时也同样恢复进入>
		if (OptionServer.cbOptionFlags==OSF_SHALL_CLOSE)
		{//禁止进入游戏：恢复进入游戏
			OptionServer.cbOptionValue=(OptionServer.cbOptionValue==TRUE)?FALSE:TRUE;
			OptionServer.cbOptionFlags=OSF_ENTER_ROOM;
			m_pIClientSocket->SendData(MDM_GR_MANAGER,SUB_GR_OPTION_SERVER,&OptionServer,sizeof(OptionServer));
		}
		//====END
