...
bool CGlobalUnits::InitGlobalUnits()
{
	...
	//设置界面目录
	_sntprintf(m_szDirSkin,sizeof(m_szDirSkin),TEXT("%s\\Skin"),m_szDirWork);
...
void CGlobalUnits::GetClientSerial(tagClientSerial & ClientSerial)
{
	...
	// 上面这样做不是没空间存硬盘标识了？
	WORD wIndex=(wMacSize-1)/sizeof(DWORD);
	LPCTSTR pszHardDisk[]={TEXT("C:\\"),TEXT("D:\\"),TEXT("E:\\"),TEXT("F:\\"),TEXT("G:\\")};
...
bool CGlobalAttemper::ShowStatusMessage(LPCTSTR pszMessage, CWnd * pNotifyWnd)
{
	...
	::SetWindowPos(m_DlgStatus.GetSafeHwnd(), HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_SHOWWINDOW);
	return m_DlgStatus.ShowStatusMessage(pszMessage,pNotifyWnd);*/
	if(m_DlgStatus.GetSafeHwnd() && pNotifyWnd)	// add by HouGuoJiang 2011-11-25
	{
		m_DlgStatus.SetForegroundWindow();
		::SetWindowPos(m_DlgStatus.GetSafeHwnd(), HWND_TOP, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_SHOWWINDOW);
		return m_DlgStatus.ShowStatusMessage(pszMessage,pNotifyWnd);
...
bool CGlobalAttemper::DestroyStatusWnd(CWnd * pNotifyWnd)
{
	bool bRes = false;
	if(m_DlgStatus.GetSafeHwnd() && pNotifyWnd)	// add by HouGuoJiang 2011-11-25
		bRes = m_DlgStatus.DestroyStatusWnd(pNotifyWnd);
	return bRes;
	//return m_DlgStatus.DestroyStatusWnd(pNotifyWnd);
}

//下载游戏
bool CGlobalAttemper::DownLoadClient(LPCTSTR pszKindName, WORD wKindID, bool bDisplay, WORD wType)
{
	//构造数据
	...
	_sntprintf(DownLoadRequest.szFileName,sizeof(DownLoadRequest.szFileName),TEXT("%s.exe"),pszKindName);
	_sntprintf(DownLoadRequest.szLocalPath,sizeof(DownLoadRequest.szLocalPath),TEXT("%s\\DownLoad"),g_GlobalUnits.GetWorkDirectory());
 	//_sntprintf(DownLoadRequest.szDownLoadUrl,sizeof(DownLoadRequest.szDownLoadUrl),TEXT("http://192.168.1.105/Download.asp?KindID=%ld&LocalVersion=0&PlazaVersion=%ld"),wKindID,VER_PLAZA_FRAME);
	//_sntprintf(DownLoadRequest.szDownLoadUrl,sizeof(DownLoadRequest.szDownLoadUrl),TEXT("http://192.168.1.100/jw/PaoHuZi.exe"),wKindID,VER_PLAZA_FRAME);
	//_sntprintf(DownLoadRequest.szDownLoadUrl,sizeof(DownLoadRequest.szDownLoadUrl),TEXT("http://192.168.1.100/Download.asp?KindID=%ld&LocalVersion=0&PlazaVersion=%ld"),wKindID,VER_PLAZA_FRAME);

	//_sntprintf(DownLoadRequest.szDownLoadUrl,sizeof(DownLoadRequest.szDownLoadUrl),TEXT("http://192.168.1.108/qpgame/download/%s"),DownLoadRequest.szFileName);
	_sntprintf(DownLoadRequest.szDownLoadUrl,sizeof(DownLoadRequest.szDownLoadUrl), CInitData::s_cLoadServer, DownLoadRequest.szFileName);

	DownLoadRequest.byDownLoadTag = wType;//wType=1下载;wType=2更新
	//投递请求
