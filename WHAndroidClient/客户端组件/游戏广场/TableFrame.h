...
interface ITableView : IUnknownEx
{
	...
	//设置页码
	virtual void __cdecl SetPage(int iPage)=NULL;
	////获得页码
	//virtual int  __cdecl GetPageNum()=NULL;
};
...
public:
	//设置页码
	virtual void __cdecl SetPage(int iPage){m_iPage=iPage;};
};

//////////////////////////////////////////////////////////////////////////

#define MAX_TABLE_VIEW_COUT				(12)
//游戏桌子框架
class CTableFrame : public ITableFrameView //public CWnd,
...
protected:
	...
	CTableView							* m_pCurTable[MAX_TABLE_VIEW_COUT];					//
...
public:
	...
	void UpdatePage();
	//桌子总数
	int GetAllLine(){return m_wTableCount;};
	//设置桌子号
	void SetPageNum(int iPageNum){m_iPageNum=iPageNum;};
	//获得桌子号
	int GetPageNum(){return m_iPageNum;}
	/// 转向指定页
	int OpenDesignationPage(int iTableID);
...
protected:
	...
	//通过桌子号得到视图位置
	WORD GetViewTableID(WORD wTableId);
	//消息函数
//protected:
public:
	//重画消息
