...
BOOL CShortMessage::OnInitDialog()
{
	...
	{
		TCHAR szTitle[256];
		_sntprintf(szTitle,sizeof(szTitle),TEXT("[ %s ] － [ %s ]"),m_pTargetUserItem->GetUserData()->szName,m_strRoomName);
	...
	_sntprintf(szTitle,CountArray(szTitle),TEXT("【温馨提示】"));
	m_MessageProxyHelper->InsertGeneralString(szTitle,RGB(255,0,0),MS_NORMAL,false);

	//预警信息
	TCHAR szMessage[128] ;
	_sntprintf(szMessage,CountArray(szMessage),TEXT("客服人员不会以任何形式向您索取您的帐户信息，请玩家提高警惕，保护好自己帐号密码！"));
...
void CShortMessage::SetTalkInfo(IUserItem * pTargerUserItem, TCHAR * szRoomName)
{
	...
	if (GetSafeHwnd()!=NULL)
	{
		//设置标题
		TCHAR szTitle[256];
		_sntprintf(szTitle,sizeof(szTitle),TEXT("[ %s ] － [ %s ]"),m_strTargerName,m_strRoomName);
		SetWindowText(szTitle);
...
{
	//AfxMessageBox(TEXT("CShortMessage::OnOK"));
...
void CShortMessage::OnRecvMessage(TCHAR * szSendName, TCHAR * szMessage, COLORREF crColor, bool bMeMessage)
{
	...
	{
		TCHAR szTitle[128]=TEXT("");
		_sntprintf(szTitle,CountArray(szTitle),TEXT("[%s] %u/%u/%u %02d:%02d:%02d"),szSendName,SystemTime.wYear, SystemTime.wMonth, SystemTime.wDay,SystemTime.wHour,SystemTime.wMinute,SystemTime.wSecond);
