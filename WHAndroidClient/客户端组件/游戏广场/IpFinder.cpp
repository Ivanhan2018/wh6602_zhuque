...
// 获得IP类型，即获得中国电信、中国联通、中国铁通
int CIpFinder::GetIpType(const char* szInfo)
{
	int iRes = IP_TYPE_Unknow;
	CAtlString strInfo = szInfo;

	if(strInfo.IsEmpty())
			return iRes;

	if(strInfo.Find("电信")>=0)
		return IP_TYPE_China_Telecom;

	if(strInfo.Find("联通")>=0 || strInfo.Find("网通")>=0)
		return IP_TYPE_China_Unicom;

	if(strInfo.Find("铁通")>=0)
		return IP_TYPE_China_Railcom;
