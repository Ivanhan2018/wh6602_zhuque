...
void CGameOption::LoadOptionParameter()
{
	...
	//积分限制
	CString strProfile;
	strProfile=pApp->GetProfileString(pGameKind->szKindName,TEXT("MaxScore1"),TEXT("0"));
	m_lMaxScore=_atoi64(LPCTSTR(strProfile));
	strProfile=pApp->GetProfileString(pGameKind->szKindName,TEXT("LessScore1"),TEXT("0"));
	m_lLessScore=_atoi64(LPCTSTR(strProfile));

	//m_lMaxScore=pApp->GetProfileInt(pGameKind->szKindName,TEXT("MaxScore"),0);
	//m_lLessScore=pApp->GetProfileInt(pGameKind->szKindName,TEXT("LessScore"),0);
...
void CGameOption::SaveOptionParameter()
{
	...
	CString strProfile;
	strProfile.Format(TEXT("%I64d"),m_lMaxScore);
	pApp->WriteProfileString(pGameKind->szKindName,TEXT("MaxScore1"),strProfile);
	strProfile.Format(TEXT("%I64d"),m_lLessScore);
	pApp->WriteProfileString(pGameKind->szKindName,TEXT("LessScore1"),strProfile);
...
CGlobalOption::CGlobalOption()
{
	...
	//默认电信
	m_byNetWorkTag = 2;
	m_byCheckLineResult = 2;
	
	//默认有声音
	m_bEnableBackMusic = true;
	return;
}
