...
public:
	...
	//获取接口
	inline IModeluInterface * GetNewInterface() const;
};
...
//获取接口
template <typename IModeluInterface>
inline IModeluInterface * CTempldateHelper<IModeluInterface>::GetNewInterface() const
{
	if (m_hDllInstance==NULL) 
	{
		return NULL;
	...
	//创建组件
	return (IModeluInterface *)CreateProc(m_Guid,m_dwVersion);
}
