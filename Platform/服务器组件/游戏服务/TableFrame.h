...
protected:
	...
	DWORD                          m_endTime;           //游戏结束时间  由于扎金花游戏服务器端的数据会延迟6秒发送。所以服务提对
	                                                                    //排队系统需要考虑这个问题
...
protected:
	...
	LONGLONG						m_OriginGold[MAX_CHAIR];			//游戏积分
...
public:
	...
	CAttemperEngineSink		* m_pAttemperEngineSink;       //调度接口，彭文添加
	//函数定义
public:
	//构造函数
	CTableFrame();
...
public:
	...
	//彭文添加
	//得到所有的空椅子数目
	WORD GetTatolNullChairCount();
	//有没有玩家
	bool HasPlayer();
    //得到坐下的人数
	WORD GetSittingChairCount();
	//是否这个桌子都是机器人
	bool OnlyAndroidInDesk();
	//得到机器人的数目
	WORD GetAndroidCountInDesk();
	//得到普通玩家的数目
	WORD GetNormalCountInDesk();
	//添加结束
...
public:
	//彭文添加， 处理任务
	//请求完成当前的一个任务
	bool __cdecl QuestTaskFinish(WORD wChairID, DWORD dwGems);
private:
	//奖励的时候写入宝石
	bool __cdecl WriteUserGem(WORD  wChairID, DWORD lGem);

public:	
	//广播消息
	void BroadcastMessage(WORD wMainCmdID, WORD wSubCmdID, const void* pData, WORD wDataSize);
	//解除只有机器人的桌子
	void CancelOnlyAndroidDesk();
	//解除固定数目的机器人
	void CancelAndroid(int num);
	 //添加结束
};
