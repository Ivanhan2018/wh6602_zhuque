...
void CGameService::InitSome()
{
	...
	if(S_OK==hResult && m_pRoomLog)
	{
		...
		strTmp.Format(".\\RoomLog\\%s（%d）\\", 
								m_AttemperEngineSink.m_pGameServiceOption->szGameRoomName, 
								m_AttemperEngineSink.m_pGameServiceOption->wServerID);
		strPath = strPath.Left(strPath.Find("ServiceLoader.exe")) + strTmp;

		m_pRoomLog->Init();
		m_pRoomLog->Start();
		m_pRoomLog->Register(strPath, 50000);

		m_pRoomLog->AddMsg(0, 0, TRUE,"***** 日志开启");
	}
	else
	{
...
void CGameService::UninitSome()
{
	// 日志
	if (m_pRoomLog)
	{
		if (m_pRoomLog)
			m_pRoomLog->AddMsg(0, 0, TRUE,"***** 日志关闭");
...
bool __cdecl CGameService::StartService(tagGameServiceParameter * pGameServiceParameter)
{
	...
	//终止服务
	//StopService();	// 这种写法是谁教的，简直是不明理就的滥用！！！？？？
	                            // StopService 是严格放在“停止”或“结束”时才用的
	...
	//调整参数
	WORD wLessConnect=m_GameServiceOption.wTableCount*m_GameServiceAttrib.wChairCountMax;
	...
	InitSome();
	m_AttemperEngineSink.SetRoomLog(m_pRoomLog);
	m_DataBaseEngineSink.SetRoomLog(m_pRoomLog);
