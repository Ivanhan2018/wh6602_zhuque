...
#define IDI_UPDATE_AFFICHE              (IDI_ATTEMPER_START+10)             //彭文添加，更新公告
#define IDI_UPDATE_BUSY_DESK            (IDI_ATTEMPER_START+11)             //彭文添加，更新繁忙的桌子
#define IDI_UPDATE_ADD_ANDROID          (IDI_ATTEMPER_START+12)             //彭文添加，自动加入机器人
#define IDI_UPDATE_ADD_USER             (IDI_ATTEMPER_START+13)             //彭文添加，尝试将用户加入到游戏
#define IDI_KEEP_SITTING_ANDROID        (IDI_ATTEMPER_START+14)             //彭文添加，尝试加入站位机器人
#define IDI_UPDATE_HAPPY_MESS           (IDI_ATTEMPER_START+15)             //彭文添加，更新喜报   
#define IDI_LOAD_GRADE_INFO             (IDI_ATTEMPER_START+16)             //彭文添加，更新等级
#define IDI_SEND_HAPPY_MESS             (IDI_ATTEMPER_START+17)             //彭文添加，发送喜报给用户
#define IDI_UPDATE_EXIT_USER            (IDI_ATTEMPER_START+18)             //彭文添加，更新强退的用户 
#define IDI_CONTINUE_CONNECT			(IDI_ATTEMPER_START+19) 			//保持数据库连接一直存在ID		Author:<cxf>
#define IDI_NEWGIFT						(IDI_ATTEMPER_START+20) 			//新手礼包ID					Author:<cxf>
//时间定义
#define TIME_RECONNECT					15000L								//重连时间
#define TIME_UPDATE_LIST				30000L								//更新时间
#define TIME_UPDATE_ONLINE				30000L								//更新人数
#define TIME_UPDATE_AFFICHE             (1000*60*8)                         //彭文添加，每8小时更新一次系统公告     需要修改
#define TIME_UPDATE_BUSY_DESK           (1000*30)                           //彭文添加，每30秒更新一次繁忙的桌子 需要修改
#define TIME_UPDATE_ADD_ANDROID         (1000*5)                            //彭文添加，每10秒更新一次添加机器人
#define TIME_UPDATE_ADD_USER            (1000*2)                            //彭文添加，每3秒增加用户得到游戏
#define TIME_KEEP_SITTING_ANDROID       (1000*60)                           //彭文添加，每5分钟增加一次站位机器人

#define TIME_UPDATE_HAPPY_MESS          (1000*30)                            //彭文添加，喜报的更新间隔
#define TIME_LOAD_GRADE_INFO            (1000*2)                            //彭文添加，等级的更新间隔
#define TIME_SEND_HAPPY_MESS            (1000*60*5)                         //彭文添加，喜报发送间隔
#define TIME_UPDATE_EXIT_USER           (1000*30)                           //彭文添加，强退更新时间
#define TIME_CONTINUE_CONNECT			(1000*600)							//重连数据库间隔时间			Author:<cxf>
#define TIME_NEWGIFT					(1000*20)							//新手礼包计算间隔时间			Author:<cxf>
//////////////////////////////////////////////////////////////////////////
//机器时间
...
struct tagProperty
{
	...
	DWORD                               dwActiveTime;                       //道具激活时间，如：道具使用时候的时间
	                                                                        //当前道具已经激活，下次登陆时候的时间，
	                                                                        //主要是为了处理dwPastTime的计算  单位为秒
	bool                                bUse;                               //是否这个类型的道具使用中
	//添加结束
};

//寻宝任务
typedef      CMD_GF_TASK_tagTask        tagFindGemsTask;
...
struct tagServerUserData
{
	...
	//分数信息
	LONGLONG 							lStorageGold;						//存储积分 2011-10-10前叫做 lStorageScore  
	...
	//彭文添加
	tagFindGemsTask                     task;                               //彭文添加  任务结构
	TCHAR                               szNickName[NAME_LEN];               //彭文添加  昵称
	USER_TYPE                           utUserType;                         //用户类型   
	DWORD                               m_ArrangeQueueTime;                 //进入排队机的时间 
	DWORD								dwOnlineTime;						//一天在线时长
	DWORD								wOnlineGiftCount;					//一天在线赠送次数
	//添加结束
...
public:
    ...
    //获取每一局的税收
	virtual LONGLONG __cdecl GetRoundRevenue()=NULL;
	//游戏时间
	virtual DWORD __cdecl GetPlayTimeCount()=NULL;
	//每局游戏时间
	virtual DWORD __cdecl GetRoundPlayTime()=NULL;
	//在线时间
	virtual DWORD __cdecl GetOnlineTimeCount()=NULL;
	// 获得在线有礼标记
	virtual BOOL __cdecl GetOnlineTimeGolSign(BYTE byIndex)=NULL;
	// 设置在线有礼标记
	virtual void __cdecl SetOnlineTimeGoldSign(BYTE byIndex, BOOL bVal=TRUE)=NULL;
	// 获得升级送宝标记
	virtual BOOL __cdecl GetUpgradeGemSign(BYTE byIndex)=NULL;
	// 设置升级送宝标记
	virtual void __cdecl SetUpgradeGemSign(BYTE byIndex, BOOL bVal=TRUE)=NULL;
...
public:
	...
	//奖励的时候写入宝石
	virtual bool __cdecl WriteUserGem( DWORD lGem) = 0;

	// 奖励的时候写入金币
	virtual bool __cdecl WriteUserGold(LONGLONG llGold, LONGLONG llRevenue=0l)=0;

    //清除修改数据
	virtual void __cdecl ClearScorModify() = 0;
public:
...
public:
    ...
    //彭文添加
	//完成当前的一个任务
	virtual bool __cdecl QuestTaskFinish(WORD wChairID, DWORD dwGems) = 0;
	//添加结束
public:
   //任务处理函数
};
...
interface ITableFrameSink : public IUnknownEx
{
	...
	virtual bool channel_to_game_service(int iMainCode, int iAssistantCode, void* pData, int iDataLen) = NULL;
...
public:
    ...
    virtual bool __cdecl UpdateUserItem(IServerUserItem * pIServerUserItem) = 0;
	//网络接口
public:
	//发送数据
...
public:
	...
	//发送警告消息  彭文添加
	virtual void SendWarningPacket(IServerUserItem * pIServerUserItem, int MDM_MES, int SUB_MES, TCHAR* pMes) = 0;

	//将用户塞入等待队列中
	virtual bool InsertUserInQueue(IServerUserItem * pIServerUserItem) = 0;
