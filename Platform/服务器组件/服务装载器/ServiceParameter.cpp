...
BOOL CServiceParameter::OnInitDialog()
{
	...
	((CEdit *)GetDlgItem(IDC_ANDROID_DESK_COUNT))->LimitText(3); //彭文添加
	...
	lpAddrList=(LPIN_ADDR *)pHostent->h_addr_list;
	pServerAddr=(CComboBox *)GetDlgItem(IDC_SERVER_ADDR2);
	while (*lpAddrList!=NULL)
	{
		lstrcpyn(szServerAddr,inet_ntoa(**lpAddrList),CountArray(szServerAddr));
		int nIndex=pServerAddr->AddString(szServerAddr);
...
void CServiceParameter::OnOK()
{
	...
	//设置机器人站位的桌子数目  彭文添加
	OptionParameter.GameServiceOption.wAndroidTableCount=GetDlgItemInt(IDC_ANDROID_DESK_COUNT, NULL, FALSE); 
	//设置玩家等待机器人时间    彭文添加
	OptionParameter.GameServiceOption.wUserWaitUntilAndroid = GetDlgItemInt(IDC_USER_WAIT_TIME, NULL, FALSE);
	...
	//限制等级
	OptionParameter.GameServiceOption.lRestrictGrade= GetDlgItemInt(IDC_RESTRICT_GRADE,NULL, FALSE);
	//进入金钱
	OptionParameter.GameServiceOption.lEnterMoney = GetDlgItemInt(IDC_ENTER_MONEY, NULL, FALSE);
	...
	WORD wMaxConnect=m_GameServiceAttrib.wChairCountMax*OptionParameter.GameServiceOption.wTableCount;
	...
	if (nCurSel==CB_ERR)
	{
		...
		OptionParameter.GameServiceOption.dwServerAddrCNN=inet_addr(szServerAddr);
	}
	else OptionParameter.GameServiceOption.dwServerAddrCNN=(DWORD)pServerAddr->GetItemData(nCurSel);
	...
	if (nCurSel==CB_ERR)
	{
		...
		{
			AfxMessageBox(TEXT("请选择正确的服务地址"),MB_ICONINFORMATION);
			GetDlgItem(IDC_SERVER_ADDR2)->SetFocus();
			return;
		}
		OptionParameter.GameServiceOption.dwServerAddrCUN=inet_addr(szServerAddr);
	}
	else OptionParameter.GameServiceOption.dwServerAddrCUN=(DWORD)pServerAddr->GetItemData(nCurSel);
...
void CServiceParameter::UpdateOptionParameter()
{
	...
	WORD wGameGenre[2]={GAME_GENRE_MATCH,GAME_GENRE_GOLD};
	LPCTSTR pszGameGenre[2]={TEXT("比赛类型"),TEXT("金币类型")};
	...
	//机器人最低座子数目 彭文添加
	if (m_OptionParameter.GameServiceOption.wAndroidTableCount==0) SetDlgItemText(IDC_ANDROID_DESK_COUNT, TEXT(""));
	else SetDlgItemInt(IDC_ANDROID_DESK_COUNT, m_OptionParameter.GameServiceOption.wAndroidTableCount, FALSE);

	//设置玩家等待机器人时间 彭文添加
	if (m_OptionParameter.GameServiceOption.wUserWaitUntilAndroid==0) SetDlgItemText(IDC_USER_WAIT_TIME, TEXT(""));
	else SetDlgItemInt(IDC_USER_WAIT_TIME, m_OptionParameter.GameServiceOption.wUserWaitUntilAndroid, FALSE);
	...
	//等级限制
	if (m_OptionParameter.GameServiceOption.lRestrictGrade == 0) SetDlgItemText(IDC_RESTRICT_GRADE, TEXT(""));
	else SetDlgItemInt(IDC_RESTRICT_GRADE, m_OptionParameter.GameServiceOption.lRestrictGrade, FALSE);

	//进入金币
	if (m_OptionParameter.GameServiceOption.lEnterMoney ==0) SetDlgItemText(IDC_ENTER_MONEY, TEXT(""));
	else SetDlgItemInt(IDC_ENTER_MONEY, m_OptionParameter.GameServiceOption.lEnterMoney, FALSE);
	...
	for (i=0;i<nItemCount;i++)
	{
		...
		if (m_OptionParameter.GameServiceOption.dwServerAddrCNN==dwItemAddr) 
		{
	...
	{
		TCHAR szServerAddr[256]=TEXT("");
		BYTE * cbServerAddr=(BYTE *)&m_OptionParameter.GameServiceOption.dwServerAddrCNN;
	...
	//网通服务地址
	pServerAddr=(CComboBox *)GetDlgItem(IDC_SERVER_ADDR2);
	nItemCount=pServerAddr->GetCount();
	for (i=0;i<nItemCount;i++)
	{
		DWORD_PTR dwItemAddr=pServerAddr->GetItemData(i);
		if (m_OptionParameter.GameServiceOption.dwServerAddrCUN==dwItemAddr) 
		{
			pServerAddr->SetCurSel(i);
			break;
		}
	...
	{
		TCHAR szServerAddr[256]=TEXT("");
		BYTE * cbServerAddr=(BYTE *)&m_OptionParameter.GameServiceOption.dwServerAddrCUN;
		_snprintf(szServerAddr,CountArray(szServerAddr),TEXT("%ld.%ld.%ld.%ld"),cbServerAddr[0],cbServerAddr[1],cbServerAddr[2],cbServerAddr[3]);
		pServerAddr->SetWindowText(szServerAddr);
	}
