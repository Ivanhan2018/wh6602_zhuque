...
#if 0
	// CIPAddressCtrl::GetAddress 函数有问题，不可以用
...
#else
	GetDlgItem(IDC_USER_DATABASE_IP)->GetWindowText(strServerAddr);
	dwDataBaseIP = inet_addr(strServerAddr.GetBuffer());
	// 规范化的写法
	_snprintf(	InitParamter.m_szUserDataBaseAddr,sizeof(InitParamter.m_szUserDataBaseAddr),TEXT("%d.%d.%d.%d"),
					dwDataBaseIP & 0x000000ff, (dwDataBaseIP & 0x0000ff00) >> 8, (dwDataBaseIP & 0x00ff0000) >> 16, (dwDataBaseIP & 0xff000000) >> 24);
#endif
...
#else
	GetDlgItem(IDC_CENTER_SERVER_IP)->GetWindowText(strServerAddr);
	dwDataBaseIP = inet_addr(strServerAddr.GetBuffer());
	// 规范化的写法
	_snprintf(	InitParamter.m_szCenterServerAddr, 
					sizeof(InitParamter.m_szCenterServerAddr),
					TEXT("%d.%d.%d.%d"),
					dwDataBaseIP & 0x000000ff, (dwDataBaseIP & 0x0000ff00) >> 8, (dwDataBaseIP & 0x00ff0000) >> 16, (dwDataBaseIP & 0xff000000) >> 24);
#endif
