...
#define SUB_GR_LOGON_FULL           103                                 //人已经满了 彭文添加

#define SUB_GR_LOGON_AFFICHE        105                                 //公告信息   彭文添加
#define SUB_GR_HAPPY_MESS           106                                 //喜报       彭文添加
...
struct CMD_GR_LogonByUserID
{
	...
	TCHAR							szPassWord[PASS_LEN];				//登录密码
	DWORD                           dwKindID;                           //游戏id,每款游戏在数据库中都有一个id   彭文添加
};
...
//登陆房间人已经满了
struct CMD_GR_LogonFull
{
	TCHAR							szErrorDescribe[128];				//错误消息
    DWORD                           dwCountUser;                        //现在的人数
};
...
{
	DWORD							dwTargetUserID;						//目标用户
	BOOL							bCutOnline;							//踢出方式（为真断线，为假逃跑）
