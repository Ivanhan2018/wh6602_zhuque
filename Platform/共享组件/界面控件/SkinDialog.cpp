...
{
	memset(&m_NCRect, 0, sizeof(m_NCRect));
}
...
void CSkinDialog::SetNCRect(int Caption, int left, int right, int top, int bottom)
{
	m_NCRect.Caption = Caption > -1 ? Caption : m_NCRect.Caption;
	m_NCRect.left = left > -1 ? left : m_NCRect.left;
	m_NCRect.right = right > -1 ? right : m_NCRect.right;
	m_NCRect.top = top > -1 ? top : m_NCRect.top;
	m_NCRect.bottom = bottom > -1 ? bottom : m_NCRect.bottom;
}
//激活消息
...
void CSkinDialogEx::DrawPic(CDC * pDC,UINT Uid,CRect *rect)
{
	CBitmap  m_bmp;
	...
	HBITMAP hbmp = (HBITMAP)::LoadImage(AfxGetInstanceHandle(), 
		MAKEINTRESOURCE(Uid), IMAGE_BITMAP, 0, 0, LR_LOADMAP3DCOLORS);         
	m_bmp.Attach( hbmp );
	CBrush brush;
...
BOOL CSkinDialogEx::PreTranslateMessage( MSG * pMsg )
{
