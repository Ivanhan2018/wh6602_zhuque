...
//发送语音消息
bool __cdecl CClientKernel::SendLanguageListMsg(UINT uiIndex)
{
	const tagUserData* pMeInfo = GetMeUserInfo();
	if (NULL != pMeInfo)
	{
		IPC_UserLanguageInfo info;
		::memset(&info, 0, sizeof(IPC_UserLanguageInfo));
		info.dwIndex = uiIndex;
		info.dwSendUserID = pMeInfo->dwUserID;
		SendSocketData(MDM_GF_FRAME,IPC_SUB_SENDLANLISTINFO,&info,sizeof(IPC_UserLanguageInfo));
	}
...
bool __cdecl CClientKernel::KillGameTimer(UINT nTimerID)
{
	...
	if ((m_nTimerID!=0)&&((m_nTimerID==nTimerID)||(nTimerID==0)))
	{
		...
		if (m_wTimerChairID!=INVALID_CHAIR  &&  0<=m_wTimerChairID  && m_wTimerChairID<MAX_CHAIR)
		{
	...
	case IPC_MAIN_SOCKET:	//网络数据
		{
			...
			if (bSuccess==false)
			{
				SendProcessData(IPC_MAIN_CONCTROL,IPC_SUB_GAME_FRAME_CLOSED);
				m_bInquire=false;
	...
	case IPC_SUB_SOCKET_RECV:	//数据接收
		{
...
#ifdef CLIENT_KERNEL_PRINT
			char chData2[128]={0};
			sprintf(chData2, "CClientKernel::OnIPCSocket:数据接收(%d, %d).", pHead->wMainCmdID, pHead->wSubCmdID);
			m_pIMessageProxy->InsertSystemString(chData2,0);
#endif
...
#ifdef CLIENT_KERNEL_PRINT
			char chData3[128]={0};
			sprintf(chData3, "CClientKernel::OnIPCSocket:%d, %d, %d", bHandle, pSocketPackage->Command.wMainCmdID, pSocketPackage->Command.wSubCmdID);
			m_pIMessageProxy->InsertSystemString(chData3,0);
#endif
			...
			if ((bHandle==false)&&(pSocketPackage->Command.wMainCmdID==MDM_GF_FRAME))
			{
				...
				case SUB_GF_LOOKON_CONTROL:	//旁观控制
					{
						...
						if (IsLookonMode()==true)
						{
							...
							{
								m_pIMessageProxy->InsertSystemString(TEXT("您被允许观看玩家游戏"),0);
							}
							else
							{
								m_pIMessageProxy->InsertSystemString(TEXT("您被禁止观看玩家游戏"),0);
							}
							m_pIClientKernelSink->OnEventHaveChatMessage();
						}
				...
				case SUB_GR_HAPPY_MESS:		//喜报信息
					{
						//提取数据
						if (NULL != pSocketPackage)
						{
							char * pData = (char*)pSocketPackage->cbBuffer;
							if (NULL != pData)
							{
								if (NULL != m_pIClientKernelSink)
								{
									m_pIClientKernelSink->OnEventGameAfficheInfo(pData, wPacketSize);
								}
							}
						}
						return true;
					}
				case SUB_RECV_SOUND_MESS:	//语音消息
					{
						if (wDataSize<sizeof(IPC_UserLanguageInfo)) return false;
						IPC_UserLanguageInfo * pMessage=(IPC_UserLanguageInfo *)pSocketPackage->cbBuffer;
						if (NULL != pMessage)
						{
							m_pIClientKernelSink->OnEventUserLanguageInfo(pMessage->dwSendUserID, pMessage->dwIndex);
						}
						return true;
					}
				case SUB_GF_MESSAGE:		//系统消息
					{
						...
						{//插入系统消息
							m_pIMessageProxy->InsertSystemString(pMessage->szContent,MS_NORMAL);
							m_pIClientKernelSink->OnEventHaveChatMessage();
						}
						...
						if (SMT_GAMEDJINFO&pMessage->wMessageType)
						{//道具消息
							m_pIClientKernelSink->OnEventGameDJInfo(pMessage->szContent);
						}
			...
			//礼物消息	Author<cxf>; DateTime<2012-01-16>
			if ((bHandle==false)&&(pSocketPackage->Command.wMainCmdID==MDM_GF_PRESENT))
			{
				switch (pSocketPackage->Command.wSubCmdID)
				{
				case SUB_GF_NEW_GIFT://新手礼包
					{
						//验效
						ASSERT(wPacketSize==sizeof(CMD_GF_NEW_GIFT));
						if (wPacketSize!=sizeof(CMD_GF_NEW_GIFT)) return false;
						CMD_GF_NEW_GIFT * pNewGift=(CMD_GF_NEW_GIFT *)pSocketPackage->cbBuffer;
						if (NULL != pNewGift)
						{
							m_pIClientKernelSink->OnEventNewGift(pNewGift);
						}
						return true;
					}
				case SUB_GF_ONLINE_TIME: // 在线有礼--开始计时
					{
						ASSERT(wPacketSize==sizeof(CMD_GF_ONLINE));
						if (wPacketSize!=sizeof(CMD_GF_ONLINE)) return false;
						CMD_GF_ONLINE * pOnline=(CMD_GF_ONLINE *)pSocketPackage->cbBuffer;

						if (NULL != pOnline)
						{
							m_pIClientKernelSink->OnEventOnlineKeepTime(pOnline);
						}
			...
			if ((bHandle==false)&&(pSocketPackage->Command.wMainCmdID==MDM_GF_PRESENT))
			{
				...
				case SUB_GF_RESIDUAL_PROPERTY:	//剩余道具
					{//玩家本人拥有的道具列表
						...
						//}
						m_pIClientKernelSink->OnEventUserHaveDJInfo(ResidualProperty);
						return true;
					}
				case SUB_GF_PROP_ATTRIBUTE:		//道具属性
					{//道具信息列表
						//校验参数
						ASSERT(0 == wPacketSize%sizeof(tagPropertyInfo));
						if ( 0 != wPacketSize%sizeof(tagPropertyInfo) ) return false;
				...
				case SUB_GF_ACTIVE_SUCC:		//道具激活成功
					{//CMD_GF_UserProperty
						//char chDataInfo[255]={0};
						//sprintf(chDataInfo, "SUB_GF_ACTIVE_SUCC: %d==%d", wPacketSize, sizeof(CMD_GF_UserProperty));
						//MessageBox(chDataInfo);

						ASSERT(wPacketSize==sizeof(CMD_GF_UserProperty));
						if (wPacketSize!=sizeof(CMD_GF_UserProperty)) return false;

						//设置道具信息
						m_pIClientKernelSink->OnEventUserOperationDJInfo(1, pSocketPackage->cbBuffer);
						return true;
					}
				case SUB_GF_ACTIVE_FAIL_NO_PROP:	//没有道具,激活失败
					{//CMD_GF_UserProperty
						//char chDataInfo[255]={0};
						//sprintf(chDataInfo, "SUB_GF_ACTIVE_FAIL_NO_PROP: %d==%d", wPacketSize, sizeof(CMD_GF_UserProperty));
						//MessageBox(chDataInfo);
						...
						//设置道具信息
						m_pIClientKernelSink->OnEventUserOperationDJInfo(2, pSocketPackage->cbBuffer);
						return true;
					}
				case SUB_GF_ACTIVE_FAIL_STILL_USE:	//道具还在使用，激活失败
					{//CMD_GF_UserProperty
						//char chDataInfo[255]={0};
						//sprintf(chDataInfo, "SUB_GF_ACTIVE_FAIL_STILL_USE: %d==%d", wPacketSize, sizeof(CMD_GF_UserProperty));
						//MessageBox(chDataInfo);
						...
						//设置道具信息
						m_pIClientKernelSink->OnEventUserOperationDJInfo(3, pSocketPackage->cbBuffer);
						return true;
					}
				case SUB_GF_PROP_OVERDUE:		//目前的道具已经过期了
					{//use为false,代表正在使用的道具无效.num表示剩余道具数.//CMD_GF_UserProperty
						//char chDataInfo[255]={0};
						//sprintf(chDataInfo, "SUB_GF_PROP_OVERDUE: %d==%d", wPacketSize, sizeof(CMD_GF_UserProperty));
						//MessageBox(chDataInfo);
						...
						//设置道具信息
						m_pIClientKernelSink->OnEventUserOperationDJInfo(4, pSocketPackage->cbBuffer);
						return true;
					}
				case SUB_GF_PROP_UPDATE:		//道具信息更新
					{//道具奖励消息.//CMD_GF_UserProperty
						//char chDataInfo[255]={0};
						//sprintf(chDataInfo, "SUB_GF_PROP_UPDATE: %d==%d", wPacketSize, sizeof(CMD_GF_UserProperty));
						//MessageBox(chDataInfo);
						...
						//设置道具信息
						m_pIClientKernelSink->OnEventUserOperationDJInfo(5, pSocketPackage->cbBuffer);
						return true;
					}
				case SUB_GF_FLOWER_ATTRIBUTE:	//鲜花属性
					{
						//校验参数
						ASSERT( wPacketSize == sizeof(tagFlowerInfo) );
						if ( wPacketSize != sizeof(tagFlowerInfo) ) return true;
			...
			//OSC update
			if ((bHandle==false)&&(pSocketPackage->Command.wMainCmdID==MDM_GF_TASK))
			{//任务消息
				switch (pSocketPackage->Command.wSubCmdID)
				{
				case SUB_GF_FIND_GEMS_TASK_ACCEPT:			//接收任务
					{
#ifdef CLIENT_KERNEL_PRINT
						m_pIMessageProxy->InsertSystemString(TEXT("开始接收任务消息:SUB_GF_FIND_GEMS_TASK_ACCEPT"),0);
#endif
						//效验参数
						ASSERT(wPacketSize==sizeof(CMD_GF_TASK_tagTask));
						if (wPacketSize!=sizeof(CMD_GF_TASK_tagTask)) return false;

						//变量定义
						CMD_GF_TASK_tagTask * pTaskAccept=(CMD_GF_TASK_tagTask *)pSocketPackage->cbBuffer;
						if (NULL != pTaskAccept)
						{
#ifdef CLIENT_KERNEL_PRINT
							m_pIMessageProxy->InsertSystemString(TEXT("已接收任务完成消息:SUB_GF_FIND_GEMS_TASK_ACCEPT"),0);
#endif

							m_pIClientKernelSink->OnEventTaskAccept(pTaskAccept);
							return true;
						}
						return false;
					}
					break;
				case SUB_GF_FIND_GEMS_TASK_ONCEEND:		//单个任务完成
					{
						//效验参数
						ASSERT(wPacketSize==sizeof(CMD_GF_TASK_tagTaskFinish));
						if (wPacketSize!=sizeof(CMD_GF_TASK_tagTaskFinish)) return false;

						//变量定义
						CMD_GF_TASK_tagTaskFinish * pTaskAccept=(CMD_GF_TASK_tagTaskFinish *)pSocketPackage->cbBuffer;
						if (NULL != pTaskAccept)
						{
							m_pIClientKernelSink->OnEventTaskOnceFinish(pTaskAccept);
							return true;
						}
						return false;
					}
...
bool CClientKernel::OnIPCConfig(const IPC_Head * pHead, const void * pIPCBuffer, WORD wDataSize, HWND hWndSend)
{
	...
	case IPC_SUB_SERVER_INFO:	//房间信息
		{
			...
			m_ServerAttribute.wSortID = pServerInfo->wSortID;
			m_ServerAttribute.lCellScore = pServerInfo->lCellScore;
	...
	case IPC_SUB_GAMEAFFICHE_INFO:	//公告信息
		{
			return true;

			char * pData = (char*)pIPCBuffer;
			if (NULL != pData)
			{
				if (NULL != m_pIClientKernelSink)
				{
					m_pIClientKernelSink->OnEventGameAfficheInfo(pData, wDataSize);
				}
			}
			return true;
		}
...
bool CClientKernel::OnIPCUser(const IPC_Head * pHead, const void * pIPCBuffer, WORD wDataSize, HWND hWndSend)
{
	...
	case IPC_SUB_USER_COME:		//用户消息
		{
			...
			UserData.lGem = pUserInfoHead->UserScoreInfo.lGem;
			//UserData.lHonor = pUserInfoHead->UserScoreInfo.lHonor;
			//UserData.lCredit = pUserInfoHead->UserScoreInfo.lCredit;
			UserData.dwGrade = pUserInfoHead->UserScoreInfo.dwGrade;
			//UserData.dwOrder = pUserInfoHead->UserScoreInfo.dwOrder;
			//UserData.dwWinGemsToday = pUserInfoHead->UserScoreInfo.dwWinGemsToday;

			//等级称谓
			::memcpy(UserData.chGradeName, pUserInfoHead->UserScoreInfo.chGradeName, sizeof(UserData.chGradeName));
			UserData.chGradeName[CountArray(UserData.chGradeName)-1]=0;
			...
			//增加用户
			bool bIsSuc = ActiveUserItem(&UserData);
			if (bIsSuc)
			{
#ifdef CLIENT_KERNEL_PRINT
				char chPrintData[128]={0};
				sprintf(chPrintData, "%d玩家进来了----------!", UserData.dwUserID);
				m_pIMessageProxy->InsertSystemString(chPrintData, 0);
#endif
			}
	...
	case IPC_SUB_GETUSERPASSWORD:	//获得用户md5密码
		{
			//效验参数
			ASSERT(wDataSize>=sizeof(IPC_UserPassWord));
			if (wDataSize<sizeof(IPC_UserPassWord)) return false;

			//消息处理
			IPC_UserPassWord * pUserMD5PW=(IPC_UserPassWord *)pIPCBuffer;
			if (NULL != pUserMD5PW)
			{
				m_pIClientKernelSink->OnEventUserMD5PWMessage(pUserMD5PW);
			}
			return true;
		}
	case IPC_SUB_RECVACTIONINFO:	//接受动作信息
		{
			//效验参数
			ASSERT(wDataSize>=sizeof(IPC_UserActionInfo));
			if (wDataSize<sizeof(IPC_UserActionInfo)) return false;

			IPC_UserActionInfo* pUserAction = (IPC_UserActionInfo*)pIPCBuffer;
			if (NULL != pUserAction)
			{
				m_pIClientKernelSink->OnEventUserActionInfo(pUserAction->dwSendUserID, pUserAction->dwRecvUserID, pUserAction->dwActionTag);
			}
			return true;
		}
	default:
...
bool CClientKernel::OnIPCControl(const IPC_Head * pHead, const void * pIPCBuffer, WORD wDataSize, HWND hWndSend)
{
	...
	case IPC_SUB_GAMEFRAME_EXIST:	//检测游戏框架是否存在
		{
			SendProcessData(IPC_MAIN_CONCTROL,IPC_SUB_GAMEFRAME_EXIST);
			return true;
		}
	}
...
bool CClientKernel::UpdateUserItem(DWORD dwUserID, const tagUserScore * pUserScore)
{
	...
	pUserData->lMoney=pUserScore->lMoney;
	...
	//用户游戏局数 Author<cxf>;DateTime<2012-03-05>
	pUserData->lKindIDWinCount=pUserScore->lKindIDWinCount;
	pUserData->lKindIDLostCount=pUserScore->lKindIDLostCount;
	pUserData->lKindIDFleeCount=pUserScore->lKindIDFleeCount;
	pUserData->lKindIDDrawCount=pUserScore->lKindIDDrawCount;

	//osc update
	pUserData->lGem = pUserScore->lGem; //宝石
	//pUserData->lHonor = pUserScore->lHonor; //荣誉
	//pUserData->lCredit = pUserScore->lCredit;//声望
	pUserData->dwGrade = pUserScore->dwGrade; //等级
	//pUserData->dwOrder = pUserScore->dwOrder;	//名次
	//pUserData->dwWinGemsToday = pUserScore->dwWinGemsToday;	//今日赢取

	//等级称谓
	::memcpy(pUserData->chGradeName, pUserScore->chGradeName, sizeof(pUserData->chGradeName));
	pUserData->chGradeName[CountArray(pUserData->chGradeName)-1]=0;
...
void CClientKernel::UpdateGameTitle()
{
	...
	_sntprintf(szTitle,CountArray(szTitle),TEXT("%s "),WinFileInfo.GetProductName());
