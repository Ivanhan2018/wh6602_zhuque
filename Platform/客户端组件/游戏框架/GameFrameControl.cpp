...
#define IDI_MOVE_WND				2002								//移动窗口定时器
#define TIME_MOVE_WND				20									//移动窗口定时器
...
CGameFrameControl::CGameFrameControl(CGameFrameDlg * pGameFrameDlg) : CRgnDialog(IDD_FRAME_CONTROL), m_pGameFrameDlg(pGameFrameDlg)
{
	...
	m_iCountMoveIndex=0;				//移动帧
	m_iCountMoveTotal=0;				//移动总帧数
	return;
}
...
void CGameFrameControl::OnPaint()
{
	...
	CRect rect;
	GetClientRect(&rect);
	CMemDC dcMem(&dc, rect, false);	// 用单色填充背景
