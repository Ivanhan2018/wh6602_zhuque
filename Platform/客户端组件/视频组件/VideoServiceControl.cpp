...
BOOL CVideoServiceControl::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if ( IDM_UM_FLOWER <= uCmd && uCmd <= IDM_UM_FLOWER + FLOWER_COUNT )
	{
		...
		//构造信息
		CString strMsg;
		LONG lValue = 0;//CPropertyBar::m_FlowerInfo[uFlowerIdx].lPrice;
		strMsg.Format(TEXT("您选择了赠送［%s］，将会从您的银行扣除[%ld]金币。\n按“是”继续\n按“否”取消这次操作"),g_FlowerDescribe[uFlowerIdx].szName, lValue, lValue );
...
VOID CVideoServiceControl::OnBnClickedMore()
{
	...
	{
		_sntprintf(szBuffer,sizeof(szBuffer),TEXT("用户名：%s [ %s ]"),m_pUserData->szName,pszMemberOrder[m_pUserData->cbMemberOrder]);
	}
	else _sntprintf(szBuffer,sizeof(szBuffer),TEXT("用户名：%s"),m_pUserData->szName);
	UserInfoMenu.AppendMenu(0,0,szBuffer);

	//用户 ID
	_sntprintf(szBuffer,sizeof(szBuffer),TEXT("游戏 ID：%ld"),m_pUserData->dwGameID);
	UserInfoMenu.AppendMenu(0,0,szBuffer);

	//用户经验
	_sntprintf(szBuffer,sizeof(szBuffer),TEXT("经验值：%ld"),m_pUserData->lExperience);
	UserInfoMenu.AppendMenu(0,0,szBuffer);

	//用户积分
	_sntprintf(szBuffer,sizeof(szBuffer),TEXT("积分：%I64d  总局数：%ld  逃跑率：%5.2f%%"),m_pUserData->lScore,lPlayCount,dFleeRate);
	UserInfoMenu.AppendMenu(0,0,szBuffer);
	...
	if (pMeUserData->dwUserID!=m_pUserData->dwUserID)
	{
		...
		//插入菜单
		_sntprintf(szBuffer,sizeof(szBuffer),TEXT("把 [ %s ] 踢出游戏房间"),m_pUserData->szName);
