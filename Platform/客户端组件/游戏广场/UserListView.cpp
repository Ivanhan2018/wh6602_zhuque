...
bool CUserListView::InsertUserItem(IUserItem * pIUserItem)
{
	...
	if (iItem!=-1)
	{
		return UpdateUserItem(pIUserItem);
	}
...
int CUserListView::GetImageStation(tagUserData * pUserData)
{
	...
	else if (pUserData->cbMasterOrder!=0)
	{
...
LPCTSTR CUserListView::GetDisplayText(IUserItem * pIUserItem, WORD wColumnPos)
{
	...
	case DTP_USER_ID:			//用户 ID
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pIUserItem->GetUserData()->dwUserID);
			return szBuffer;
		}
	case DTP_GAME_ID:			//游戏 ID
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pIUserItem->GetUserData()->dwGameID);
			return szBuffer;
		}
	case DTP_INSURE_SCORE:		//存储金币
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%I64d"),pIUserItem->GetUserData()->lMoney);
			return szBuffer;
		}
	case DTP_GAME_GOLD:			//游戏金币
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%I64d"),pIUserItem->GetUserData()->lGameGold);
			return szBuffer;
		}
	case DTP_LOVE_LINESS:		//用户魅力
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pIUserItem->GetUserData()->lLoveliness);
			return szBuffer;
		}
	...
	case DTP_USER_TABLE:		//游戏桌号
		{
			...
			{
				_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%3d"),wTableID+1);
			}
			else 
			{
				szBuffer[0]=0;
	...
	case DTP_USER_SCORE:		//用户积分
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%I64d"),pUserData->lScore);
			return szBuffer;
		}
	case DTP_USER_EXPERIENCE:	//用户经验
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->lExperience);
			return szBuffer;
		}
	case DTP_USER_WIN:			//胜局盘数
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->lWinCount);
			return szBuffer;
		}
	case DTP_USER_LOST:			//输局盘数
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->lLostCount);
			return szBuffer;
		}
	case DTP_USER_DRAW:			//和局盘数
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->lDrawCount);
			return szBuffer;
		}
	case DTP_USER_FLEE:			//逃局盘数
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pUserData->lFleeCount);
			return szBuffer;
		}
	case DTP_USER_PLAY_COUNT:	//总局盘数
		{
			_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),pIUserItem->GetUserPlayCount());
			return szBuffer;
		}
	case DTP_WIN_RATE:			//用户胜率
		{
			if (pUserData->lWinCount>0)
			{
				...
				if (dWinRate>=0.01)
				{
					_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%5.2f%%"),dWinRate);
				}
				else
				{
					szBuffer[0]=0;
	...
	case DTP_LOST_RATE:			//用户输率
		{
			if (pUserData->lLostCount>0)
			{
				...
				if (dLostRate>=0.01)
				{
					_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%5.2f%%"),dLostRate);
				}
				else 
				{
					szBuffer[0]=0;
	...
	case DTP_DRAW_RATE:			//用户和率
		{
			if (pUserData->lDrawCount>0)
			{
				...
				if (dDrawRate>=0.01)
				{
					_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%5.2f%%"),dDrawRate);
				}
				else
				{
					szBuffer[0]=0;
	...
	case DTP_FLEE_RATE:			//用户逃率
		{
			if (pUserData->lFleeCount>0)
			{
				...
				if (dFleeRate>=0.01)
				{
					_sntprintf(szBuffer,sizeof(szBuffer),TEXT("%5.2f%%"),dFleeRate);
				}
				else
				{
					szBuffer[0]=0;
...
int CALLBACK CUserListView::SortFun(LPARAM lParam1, LPARAM lParam2, LPARAM lParamList)
{
	...
	case DTP_INSURE_SCORE:
		{
			int iCompRes=pUserData1->lMoney>pUserData2->lMoney?1:-1;
			return (pSortInfo->cbAscendSort)?iCompRes:-iCompRes;
		}
	case DTP_GAME_LEVEL:
		{
	...
	case DTP_WIN_RATE:
		{
			...
			if (lAllCount1!=0)
			{
				dWinRate1=(double)(pUserData1->lWinCount)/(double)(lAllCount1);
			}
			if (lAllCount2!=0) 
			{
				dWinRate2=(double)(pUserData2->lWinCount)/(double)(lAllCount2);
			}
	...
	case DTP_FLEE_RATE:
		{
			...
			if (lAllCount1!=0)
			{
				dFleeRate1=(double)(pUserData1->lFleeCount)/(double)(lAllCount1);
			}
			if (lAllCount2!=0)
			{
				dFleeRate2=(double)(pUserData2->lFleeCount)/(double)(lAllCount2);
			}
