...
bool CServerListManager::InsertTypeItem(tagGameType GameType[], WORD wCount)
{
	...
	try
	{
		...
		for (WORD i=0;i<wCount;i++)
		{
			...
			if (pListTypeTemp!=NULL) 
			{
				CopyMemory(pListTypeTemp->GetItemInfo(),&GameType[i],sizeof(tagGameType));
				continue;
			}
...
bool CServerListManager::InsertKindItem(tagGameKind GameKind[], WORD wCount)
{
	...
	try
	{
		...
		for (WORD i=0;i<wCount;i++)
		{
			...
			{
				CopyMemory(pListKindTemp->GetItemInfo(),&GameKind[i],sizeof(tagGameKind));
				continue;
			}
...
bool CServerListManager::InsertStationItem(tagGameStation GameStation[], WORD wCount)
{
	...
	try
	{
		...
		for (WORD i=0;i<wCount;i++)
		{
			...
			if (pListStationTemp!=NULL) 
			{
				CopyMemory(pListStationTemp->GetItemInfo(),&GameStation[i],sizeof(tagGameStation));
				continue;
			}
...
//查找子项
CListServer * CServerListManager::SearchServerItem(WORD wServerID)
{
	for (INT_PTR i=0;i<m_PtrArrayServer.GetCount();i++)
	{
		CListServer * pListServer=m_PtrArrayServer[i];
