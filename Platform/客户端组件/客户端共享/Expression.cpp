...
WORD					CExpression::m_wMaxLine=6;							//最大行数目
	...
	if (pExpressionItem!=NULL)
	{
		if (bSelectec)
		{
			pDC->FillSolidRect(iXPos*(EP_PIC_WHIDE+PIC_SPACE)+PIC_SPACE/2,iYPos*(EP_PIC_HIGHT+PIC_SPACE)+PIC_SPACE/2,
				EP_PIC_WHIDE+PIC_SPACE,EP_PIC_HIGHT+PIC_SPACE,RGB(91,47,18));
		...
		_snprintf(szFileName,sizeof(szFileName),TEXT("%s\\Face\\%d.bmp"),m_szPath, nFaceIndex);
		CImage ImageFace;
		if ( SUCCEEDED(ImageFace.Load(szFileName) ) )
		{
			...
			TransparentBlt(pDC->GetSafeHdc(), iXPos*(EP_PIC_WHIDE+PIC_SPACE)+PIC_SPACE,iYPos*(EP_PIC_HIGHT+PIC_SPACE)+PIC_SPACE, 
				EP_PIC_WHIDE, EP_PIC_HIGHT, ImageFace.GetDC(), 0, 0, EP_PIC_WHIDE, EP_PIC_HIGHT, RGB(79, 41, 0));
			ImageFace.ReleaseDC();
		}
	}
	return;
...
void CExpression::OnPaint()
{
	...
	dc.FillSolidRect(&rcClient,RGB(91,47,18));
	dc.Draw3dRect(&rcClient,RGB(91,47,18),RGB(91,47,18));

	//绘画表情头像
	for (WORD i=0;i<m_wMaxLine*m_wMaxRow;i++)
	{
		DrawItem(&dc,i%m_wMaxRow,i/m_wMaxRow,(m_wSelectIndex==i));
	}
