...
BOOL CalculateMD5(const char *sFilename, unsigned char acMD5Digest[])
{
	...
	while (iProcessedLength + BUFFER_SIZE <= iFileSize)
	{
		...
		//Sleep(min((iCPUTick1 - iCPUTick2) / 2, 10));
		...
		//Sleep(min((iCPUTick2 - iCPUTick1) / 2, 10));
...
BOOL CalculateStringMD5(const char* sString, unsigned int nLen, unsigned char acMD5Digest[])
{
	if (NULL == sString || NULL == acMD5Digest)
	{
		return FALSE;
	}
	
	MD5_CTX md5;
	md5.MD5Update((unsigned char *)sString, nLen);
	md5.MD5Final(acMD5Digest);
