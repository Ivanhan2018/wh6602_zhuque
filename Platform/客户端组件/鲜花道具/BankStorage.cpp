...
void CBankStorage::OnBnClickedOk()
{
	...
	{
		CString strMessage;
		if(m_lStorageGold>0)strMessage.Format(TEXT("您输入的金币值必须在1和%I64d之间"), m_lStorageGold);
		else strMessage.Format(TEXT("您的存储金币数目为0,不能进行提取操作!"));
	...
	{
		CString strMessage;
		if(m_lGameGold>0)strMessage.Format(TEXT("您输入的金币值必须在1和%I64d之间"),m_lGameGold );
		else strMessage.Format(TEXT("您的当前金币数目为0,不能进行存储操作!"));
		ShowInformationEx(strMessage,0 ,MB_ICONINFORMATION,TEXT("系统消息")) ;
...
void CBankStorage::UpdateView()
{
	...
	m_lStorageGold=m_pMeUserData->lMoney;
